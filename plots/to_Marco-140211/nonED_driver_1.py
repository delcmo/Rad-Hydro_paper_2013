...
#   check_left = scipy.integrate.odeint(DfRED_DM, fRED_left[0] + delta_fRED_left, Mach_left[1:50], mxstep = 5000, atol = int_atol, rtol = int_rtol)
#   check_left = scipy.append(fRED_left[0], check_left)
  print '2. * numpy.pi * fden_interp(Mach_left[1]) =', 2. * numpy.pi * fden_interp(Mach_left[1])
  ...
  print >> log_file, 'LEFT INTEGRATION COMPLETE'
  log_file.close()
...
#    check_right = scipy.integrate.odeint(DfRED_DM, fRED_right[0] + delta_fRED_right, Mach_right[1:50], mxstep = 5000, atol = int_atol, rtol = int_rtol)
#    check_right = scipy.append(fRED_right[0], check_right)
  print '2. * numpy.pi * fden_interp(Mach_right[1]) =', 2. * numpy.pi * fden_interp(Mach_right[1])
  ...
  if b[-1] > b[0]:
    print 'b[-1] > b[0]'
    log_file = open('collect_data.log', 'a+')
    print >> log_file, 'b[-1] > b[0]'
    log_file.close()
    b = scipy.integrate.odeint(DfRED_DM, .9999 * 2. * numpy.pi * fnum_interp(Mach_right[1]), Mach_right[1:], mxstep = 5000, atol = int_atol, rtol = int_rtol)
    print 'b[0] =', b[0]
    print 'b[-1] =', b[-1]
  if b[-1] > b[0]:
    print 'b[-1] > b[0]'
    b = scipy.integrate.odeint(DfRED_DM, .999 * 2. * numpy.pi * fnum_interp(Mach_right[1]), Mach_right[1:], mxstep = 5000, atol = int_atol, rtol = int_rtol)
    print 'b[0] =', b[0]
    print 'b[-1] =', b[-1]
  if b[-1] > b[0]:
    print 'b[-1] > b[0]'
    b = scipy.integrate.odeint(DfRED_DM, .99 * 2. * numpy.pi * fnum_interp(Mach_right[1]), Mach_right[1:], mxstep = 5000, atol = int_atol, rtol = int_rtol)
    print 'b[0] =', b[0]
    print 'b[-1] =', b[-1]
  if b[-1] > b[0]:
    print 'b[-1] > b[0]'
    b = scipy.integrate.odeint(DfRED_DM, .9 * 2. * numpy.pi * fnum_interp(Mach_right[1]), Mach_right[1:], mxstep = 5000, atol = int_atol, rtol = int_rtol)
    print 'b[0] =', b[0]
    print 'b[-1] =', b[-1]
  if b[-1] > b[0]:
    print 'the pre-factor to the right fRED integration is already 0.9'
    log_file = open('collect_data.log', 'a+')
    print >> log_file, 'the pre-factor to the right fRED integration is already 0.9'
    log_file.close()
  fRED_right = scipy.append(fRED_right[0], b)
  print 'RIGHT INTEGRATION COMPLETE'
  log_file = open('collect_data.log', 'a+')
  print >> log_file, 'RIGHT INTEGRATION COMPLETE'
  log_file.close()
...
print 'x-left integration'
log_file = open('collect_data.log', 'a+')
print >> log_file, 'x-left integration'
log_file.close()
x_left = scipy.integrate.odeint(dx_dM_left, 0, Mach_left1[1:][::-1], mxstep = 5000, atol = int_atol, rtol = int_rtol)
x_left1 = x_left.transpose()[0,::-1]
x_left1 = scipy.append(min(3. * x_left1[0], x0), x_left1)
# x_left1 = x_left - x_left[-1]
# x_left1 = x_left1.transpose()[0,:]

print 'x-right integration'
log_file = open('collect_data.log', 'a+')
print >> log_file, 'x-right integration'
log_file.close()
x_right = scipy.integrate.odeint(dx_dM_right, 0, Mach_right1[1:][::-1], mxstep = 5000, atol = int_atol, rtol = int_rtol)
x_right1 = x_right.transpose()[0,:]
x_right1 = scipy.append(x_right1, max(3. * x_right1[-1], x1))
# x_right1 = x_right - x_right[-1]
# x_right1 = x_right1.transpose()[0,:]

print 'x_left1[0] =', x_left1[0]
print 'x_right1[-1] =', x_right1[-1]
print 'sum(numpy.diff(x_left1) < 0) =', sum(numpy.diff(x_left1) < 0)
print 'sum(numpy.diff(x_right1) < 0) =', sum(numpy.diff(x_right1) < 0)
log_file = open('collect_data.log', 'a+')
print >> log_file, 'x_left1[0] =', x_left1[0]
print >> log_file, 'x_right1[-1] =', x_right1[-1]
print >> log_file, 'sum(numpy.diff(x_left1) < 0) =', sum(numpy.diff(x_left1) < 0)
print >> log_file, 'sum(numpy.diff(x_right1) < 0) =', sum(numpy.diff(x_right1) < 0)
log_file.close()
