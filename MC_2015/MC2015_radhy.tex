...
If we can prove that the right-hand side of \eqt{eq:entro_eq_all_terms} is positive, than the proposed viscous regularization, shown in the hyperbolic portion of the GRH equations in \eqt{eq:regularized_hyperbolic_GRH}, will
...
The second term on the right-hand side of \eqt{eq:entro_eq_all_terms} can be recast by noting that $s_\epsilon = \frac{1}{\rho} \frac{d\tilde{s}(\epsilon)}{d \epsilon}$ which yields:
%
\begin{equation}
