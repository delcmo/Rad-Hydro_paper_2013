...
Apply the entropy viscosity method to the $1$-D grey radiation-hydrodynamic equations $\rightarrow$ multi-physic problem with source terms.
\end{block}
...
\item Relaxation term in the energy and radiation equations: $ \sigma_a c \left( a T^4 - \epsilon \right)$.% behaves like a diffusion term when $\sigma_a \to \infty$ \cite{ShiJin}.
\item Diffusion term: $\partial_x \left( \frac{c}{3 \sigma_t} \partial_x \epsilon \right)$.
...
%************************************************
\begin{frame}
\begin{block}{Derivation of the dissipative terms:}
\begin{enumerate}
\setlength{\itemsep}{15pt}
...
\begin{block}{}
%\begin{itemize}
two viscosity coefficients: $\mu$ and $\kappa$. In the rest of this presentation, we will assume that $\kappa = \mu$ $\rightarrow$ parabolic regularization.
%\end{itemize}
\end{block}
\end{frame}
%************************************************
...
\begin{frame}{}
\begin{block}{Definition of the viscosity coefficient $\kappa$:}
\begin{align}
...
\begin{align}
&J = \max \left(| u |  [[P]],  |u| c_{Euler}^2 [[\rho]] \right) \nonumber \\
...
In the radiation equation, two second-order terms: diffusion and dissipative terms.
\begin{align}
...
\item Second-order in time (BDF2) and space (linear polynomials).
\item RHEA: Radiation-Hydrodynamic EquAtion ($1$-D code). 
\end{itemize}
\end{block}
\end{frame}
%************************************************
...
\begin{figure}[H]
        ...
        \begin{subfigure}[b]{0.4\textwidth}
                \centering
                \includegraphics[width=\textwidth]{../Mach_1p05_nel_500_density.png}
%                \caption{Pressure}
        \end{subfigure}%
