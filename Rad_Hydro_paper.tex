...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we extend the entropy viscosity method \cite{jlg1, jlg2, valentin} \comment{\color{blue}{no JLG citations?}} to the $1$-D radiation-hydrodynamic equations in a staged process. First, the reader is guided through the main steps that lead to the derivation of the dissipative terms, using the entropy minimum principle \cite{entropy}. Then, a definition for the entropy viscosity coefficient based upon the entropy production is given. \\
...
\end{equation}
where $U$ is the vector solution, $F$ is a conservative vector flux and $S$ is a vector source that contains the relaxation source terms and non-conservative terms:
\begin{equation}
U = \left[
\begin{array}{cccc}
\rho \\
...
The integral on $\partial \Omega$ requires to compute the flux $F(U)$ on the boundary. It is chosen to treat the boundary for each physics independently and details will be given in \sect{sec:simulations}. \\
