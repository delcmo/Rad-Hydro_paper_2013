...
We now propose to solve the $1$-D radiation-hydrodynamics equations by using \emph{the entropy viscosity method}. This technique was developed by Guermond et al.  for hyperbolic system of equations \cite{jlg1} and consists in
...
\end{equation}
\comment{$P_{\rho}$ has units of $P/\rho$ = units of $e$; $\frac{P}{\rho}P_e $ has units of $P^2/(\rho e)$ = units of $P$ = units of $\rho e$. Something is wrong here. Should the second term be $\frac{P}{\rho^2}P_e $?\\}
...
The definition of the second order viscosity coefficient, $\kappa_e(x,t)$, is based upon the entropy residual (\eqt{eq:equation6}) that can be recast as a function of the pressure $P$, the density $\rho$ and the radiation density energy $\epsilon$ (\app{app:appendixA}):
\begin{equation}
\label{eq:equation9}
\tilde{D}_e(x,t) = \frac{s_e}{P_e} \underbrace{ \left( \frac{dP}{dt} - c_m^2 \frac{d\rho}{dt} + \frac{d\epsilon}{dt} \right)}_\textrm{$\hat{D}_e(x,t)$}
\end{equation}
\comment{the reader will not be able to see the connection with the appendix because in the appendix there are no such equations as Eq.(10). If you recall in the main text a result from the appendix, it should be found in the appendix\\}
...
\begin{remark}
The coefficient $\rho c_m^2$ is used as a normalization parameter to make the ratio $\hat{D}_e(x,t) / \rho c_m^2$ have the unit of inverse time. The normalization factor has to be larger than $h$ in order to conserve the high-order accuracy. \comment{can you proove or provide evidence for this?}
 \end{remark}
 \begin{remark}
...
\comment{Expect for a quick look at the appendix, I have stopped here and will resume for this point onward soon}
...
\comment{I think you still had a mistake. Check my correction.}
\begin{equation}
\label{eq:app_equ2}
