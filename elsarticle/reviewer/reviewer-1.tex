...
We added a section titled ``Scaling of the entropy function derived for the non-equilibrium RHD equation in the Equilibrium-Diffusion Limit''  to show that the sound speed we use in the definition of the first-order viscosity coefficient corresponds to the maximum value of the sound speed (obtained for $\Gamma = 1/3$) derived in Ref. [11] and recalled in Eq.\tcr{(25a)}.
\bigskip
...
The reviewer is correct when stating that the system (3) cannot be written in a conservation form. Nevertheless, the system (3) is proved to be hyperbolic in the paper since its eigenvalues, given in Eq.\tcr{(4)}, are unconditionally real which allows us to assume the existence of an entropy function.
