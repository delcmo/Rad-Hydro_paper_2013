...
The reviewer is correct: the system (3) is not the frozen approximation and should not be referred to as such. We have corrected this. 
\bigskip
...
The reviewer is correct when stating that the system (3) cannot be written in a conservation form. Nevertheless, the system (3) is proved to be hyperbolic in the paper since its eigenvalues, given in Equ. 4, are unconditionally real which allows us to assume the existence of an entropy function.
\tcr{not finished.}
\bigskip
...
In the section ``Scaling of the entropy function derived for the non-equilibrium RHD equation in the Equilibrium-Diffusion Limit'', an asymptotic limit is performed to demonstrate that the entropy we selected for the system (3) match the entropy $s^*$ derived in Ref. [11] in the equilibrium-diffusion limit. \tcr{We believe this is a strong argument for using system (3) (the hyperbolic portion of system (1))}
\bigskip
...
We added a test case in the section ``An equilibrium-diffusion test'' in the results section to discuss the test case described by the reviewer. \tcr{why so smooth???}
\bigskip
