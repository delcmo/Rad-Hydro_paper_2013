...
% The equilibrium diffusion limit is also investigated with a Mach $1.05$ test case.
\end{abstract}
%
\begin{keyword}
...
%Any solution technique for the Euler equations may be applied to these equations. Thus, one may develop approximate Riemann solvers for these equations and apply them in a more general context. 
%
...
%After some algebra, given in \app{app:appendixA}, an equation satisfied by the entropy $s$ is obtained:
%\begin{equation}
...
Finally, we note that the viscous regularization obtained here in the equilibrium-diffusion limit by means of an asymptotic study of the non-equilibrium GRH equation is the same one that would have been obtained had we statred with the equilibrium-diffusion system of equations without dissipative terms and sought to regularize it. We leave this proof to the reader; the scaling of $\Pe$ as $O(1)$ is the key element leading to that result. 
% This proof is provided in \app{app:appendixB} where we demonstrate that the same viscous regularization can also be obtained by stabilizing the equilibrium-diffusion limit equations.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Note that the non-dimensionalized $\beta'$ is equal to one since . $\beta_\infty$ is the reference value for $\beta$ and has the same units as $s_0 / \epsilon^\frac{3}{4}$ as shown in \app{app:appendixA}. 
