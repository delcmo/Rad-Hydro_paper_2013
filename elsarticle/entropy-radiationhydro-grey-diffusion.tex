...
\end{equation}
%
%The difference between the two entropy expressions given in \eqt{eq:ent-scaled-2} and \eqt{eq:ent-reos} lies in the parameter $\beta_\infty$ that remains to be determined. 
...
%and the expression for $\kappa_\infty$ (\eqt{eq:kappa_infty}), we obtain that:
%
\begin{equation}
...
\rho{-2}\partial_{\rho} \left( \rho^2 \partial_{\rho} s \right) & \partial_{\rho,e} s & \partial_{\rho} \left( \rho \partial_{\epsilon} s \right) \\
 ...
 \right]
 \end{equation}
 In order to show that an entropy minimum principle holds, the signs of the terms $(a)$, $(b)$ and $(c)$ in \eqt{eq:app_entr_eq_non_equil} need to be studied.\\
...
and 
\begin{equation} 
\left( \rho s^*_\rho - e^* s^*_{e^*}  \right)  f^* + s^*_{e^*} \tilde h^* = s^*_{e^*}   \kappa^* \partial_x (\rho  e^*) \,.
% \tilde h^* = \,.
\end{equation}
\end{subequations}
%
...
To demonstrate that the viscosity coefficients are equivalent, we inspect the first-order viscosity and entropy viscosity expressions. The first-order formulae require the vale $|u| + c_m$ and we have shown that the soundspeed of the non-equilibrium equations tend to be EDL soundspeed,  \eqt{eq:soundspeed} and  \eqt{eq:soundspeedEDL}; this establishes equivalency of the first-order viscosities.

For the entropy viscosities to be equivalent, we need to show that the entropy residuals are equivalent. Starting from \eqt{eq:entro_ELD_2}, along with the definitions of \eqts{eq:functional_forms}, one obtains:
%
\begin{equation} \label{eq:entro_ELD_2}
\rho \matder {s^*}  
