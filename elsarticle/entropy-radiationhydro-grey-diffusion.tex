...
\tcr{you are mixing 1D spatial derivatives and grad/div operator here.}
...
\begin{equation}
\partial_{t'} \left( \rho' \right) + \partial_{x'}\left( \rho' u' \right) = \Pe \div \left( \kappa' \partial_{x'} \rho' \right) 
\end{equation}
%
\begin{equation}
...
where:
%
\tcr{you have $\Ls$ and $L_\infty$. They look a lot the same. Is this what you wanted? Furthermore, I had to change the definition of $\Ls$ because it was not unitless.}
\begin{multline*}
\label{eq:scaled-nb}
...
%
The parameter $\Lsi$ represents the ratio of the total mean-free-path to the scattering mean-free-path \tcr{your definition had $\sigma_a$} and scales as $O(1)$.\tcr{Morel's notes have $O(\varepsilon)$ which I believe is wrong} 
%
...
%
\begin{equation}
\partial_t \rho_0 + \partial_x \left( \rho u \right)_0 = \partial_x \left( \kappa \partial_x \left( \rho \right)\right)_0  \ ,
\end{equation}
%
\begin{equation}
...
%
\end{subequations}
\tcr{say that this is exactly the viscous reg one would have obtained when starting from the EDL\\}
%
