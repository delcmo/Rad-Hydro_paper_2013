...
\end{equation}
\tcb{We need to keep the form $s( \rho, e, \epsilon) = s_{Euler}(\rho, e) + \frac{\rho^{(0)}}{\rho }s_{rad}(\epsilon)$, otherwise the non-diagonal terms of the last row and column of the $3 \times 3$ matrix do not zero out.}
\end{subequations}
...
s' \left( \rho', e', \epsilon' \right) = s'_{Euler} \left( \rho', e' \right)+ \Re \frac{\rho'^{(0)}}{\rho'} \epsilon^{\prime,3/4}
\end{equation}
%
Remembering that $\Re = O(1)$ and that $T_0^4 = \epsilon_0$, the leading-order entropy yields (the superscript are omitted again for brevity):
%
