...
\end{equation}
with $P_x$ the standard shorthand notation for $\partial_x P$, $c^2_{Euler}$ denotes the definition of the speed of sound when considering only the 1-D Euler equations and $c^2_{rad}$ is the radiation contribution to the sound speed $c_m^2$.
...
\right.
\end{equation}
where $s_{Euler}$ is concave with respect to the radiation energy density $\epsilon$ and $s_{rad}$ is concave with respect to the internal energy $e$ and the specific volume $\rho^{-1}$. The constant $\rho_0$ appears for dimensionality purposes and will be derived further in \sect{sect:equ-diff} when performing an asymptotic study to derive the equilibrium-diffusion limit. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{align}
%
The first-order material and radiation equation along with $T_0 = \epsilon_0$ yield: $\epsilon_1 = T_1$. The total energy equation is obtained by taking the second-order material and radiation energy equations and summing them to cancel the second-order relaxation terms:
%
\begin{equation}
...
s \left( \rho, e \right)_0 = s\left( \rho, e \right)_{Euler,0} + \frac{\rho^{(0)}}{\rho_0} T_0^4 \ .
\end{equation}
%
