...
def compute_energy_diff(x_offset, x_coord, mat_density, x_coord_exact, mat_density_exact, quad_order, interp_kind):
  # SET VARIABLES
...
#    index_left = index_left-10 if index_left !=0 else index_left
    index_left = max(index_left-10,0)
    index_right = index_left
    while (x_coord_exact_offset[index_right] < node_right):
      index_right += 1
#    index_right = index_right+2 if index_right <= nb_nodes_exact-1  else nb_nodes_exact-1
    index_right = min(index_right+10, nb_nodes_exact-1)
