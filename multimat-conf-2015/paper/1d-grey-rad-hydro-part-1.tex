...
%-------------------------
\begin{abstract}
In \cite{our_jcp_radhy_paper}, the entropy viscosity method, a viscous regularization technique 
...
The dissipative fluxes added by the viscous regularization are consistent with the entropy minimum principle. 
...
\end{equation}
%
where $\Omega$ is an open convex set and $U$ is a vector solution. It is assumed that \eqt{eq:nc-syst-eq} cannot be written in a conservative form, i.e. the matrix $A(U)$ is not the jacobian matrix of vector-values function $f: \mathbb{R}^n \to \mathbb{R}^n$ such as $A(U) = \frac{\partial f(U)}{\partial U}$. System of equations of same type as \eqt{eq:nc-syst-eq} are often encountered in two-phase flow models (REFs) for instance. Under the above assumptions, the non-conservative product $A(U) \partial_x U$ is not defined by the theory of distribution (REF) when considering a Riemann problem: we cannot rigorously define the notions of weak and entropy solutions as it is done for conservation laws (REFs). In the theory proposed by DLM, the non-conservative product is defined as a bounded Borel measure by introducing a Lipschitz family of path $\phi: [0,1] \times \mathbb{R}^n \times \mathbb{R}^n$ such as $\phi(0, U_L, U_R) = U_L$ and $\phi(1, U_L, U_R) = U_R$. Roughly speaking, the idea is to specify the values of the solution at points of discontinuity so that the non-conservative product admits a weak solution. The Borel measure of the non-conservative product is denoted by $\left[ A(U) \partial_x U \right]_\phi$ and is defined as
%
\begin{equation}
...
\end{equation}
%
when the solution admits a discontinuity. The vector solutions $U_L$ and $U_R$ denote the left and the right limits of the discontinuity, respectively. This formulation of the non-conservative product allows to introduce the notion of Rankine-Hugoniot jump relation, and furthermore, to generalize the notion of shock, rarefaction and contact waves as for the conservative case. Interestingly, the DLM theory is shown to also be valid for conservative system of equations, as the regular Rankine-Hugoniot jump relation is recovered when assuming that the matrix $A(U)$ is a jacobian matrix. In other words, the case of conservative system of equations can be seen as a subset of the non-conservative system of equations. Another approach that is of interest for this paper, consists in considering the vanishing viscosity solution of \eqt{eq:nc-syst-eq} by adding a parabolic regularization as follows:
%
\begin{equation}\label{eq:nc-syst-eq-visc}
\partial_t U^\epsilon + A(U^\epsilon) \partial_x U^\epsilon = \epsilon \partial_{xx} U^\epsilon \ , \nonumber
\end{equation}
%
where $\epsilon$ is a vanishing viscosity coefficient. This approach was first studied by Bianchini et al. (REF) and further generalized by Alouges et al. (REF). Through their work, they were able to show that the solution to the vanishing viscosity system as $\epsilon \to 0$ is unique as stated in Theorem 1 on page 229 of (REF). Moreover, Le Floch in (REF) generalized the notion of entropy condition to non-conservative system of equations in the space of bounded function of bounded variation, by introducing an entropy inequality in order to ensure convergence of the numerical solution to the entropy weak solution. 

This generalization of the entropy inequality to non-conservative hyperbolic system of equations is of great interest for the focus of this paper and the application of the Entropy Viscosity Method (EVM): Delchini et al. \cite{our_jcp_radhy_paper}  proposed to solve the non-equilibrium Grey Radiation-Hydrodynamics equations by stabilizing the numerical discretization using the EVM, developed by Guermond et al. for hyperbolic systems of equations \cite{jlg1, jlg2}.
...
that the entropy minimum principle holds, which requires an entropy inequality (we define later in the introduction what we mean by \emph{entropy minimum principle}). Viscosity coefficients modulate the magnitude of the added dissipation such that it is large in shock regions and vanishingly 
...
Because of the similarity between Euler equations and the radiation-hydrodynamic equations, it is conjectured in \cite{our_jcp_radhy_paper} that the entropy 
...
adding dissipative terms to the governing laws, one must verify that the entropy minimum principle still holds in order to single out the entropy solution. Indeed, the functional form of these dissipation terms
...
The aim of this paper is to enhance the theoretical grounds for using the EVM to solve the Radiation-Hydrodynamic equations by proving the omitted diffusion
and relaxation terms do not negatively affect the previously obtained dissipative terms and thus, still satisfies the entropy condition for non-conservative system of equations as stated in REF. The remainder of the paper is as follows: in \sct{sec:GRH}, we recall
the 1-D non-equilibrium Grey Radiation-Hydrodynamic (GRH) equations along with its mathematical properties and some aspects of the theory related to non-conservative system of equations. The viscous regularization of the GRH equations based on their hyperbolic parts is recalled in \sct{sec:VR_old}.
...
After detailing the discretization and solution techniques in \sct{sec:discr}, 1-D numerical results (Mach 1.05 and 3) are given in \sct{sec:num-rslt} and plotted against semi-analytical solutions obtained from REF. Test cases with constant and temperature-dependent cross sections are considered for pure absorber (the scattering cross section is assumed to be zero). Convergence rates of the L$_1$ and L$_2$ norms are also presented for smooth numerical solutions (Mach 1.05) and numerical solutions with steady-state shocks (Mach 3).
\tcb{add something in the introduction about the ED limit}
%
%------------------------------------------------------------------
%------------------------------------------------------------------
...
%------------------------------------------------------------------
%
In this section, we recall the theoretical background and the main results obtained in \cite{our_jcp_radhy_paper}. These will serve as the foundation for the 
application of the viscous regularization to the full non-equilibrium grey Radiation-Hydrodynamic equations (GRH) equations, 
later derived in in \sct{sec:VR_new}. 

First, the 1-D  non-equilibrium grey Radiation-Hydrodynamic equations \emph{without} viscous regularization are given in \eqt{eq:GRH} along with some mathematical properties of the convective systems (first-order derivative terms). We also recall theorems of interest for this paper related to weak and entropy solutions for non-conservative system of equations. Then, the reader is guided through the main steps that were detailed in \cite{our_jcp_radhy_paper} to derive the viscous regularization using an entropy condition. At last, the 1-D non-equilibrium grey Radiation-Hydrodynamic equation \emph{with} viscous regularization are recalled.
...
\end{subequations}
%
where $\rho$, $\rho u$ and $\rho E$ are the material density, material momentum and material total energy, respectively, and $\epsilon$ denotes the radiation energy density. In order to close the system of equations given in \eqt{eq:GRH} (the system consists of five variables and four partial differential equations), a closure relation is required to compute the material pressure $P$ (see \eqt{eq:GRHmom} and \eqt{eq:GRHenerg}) under the form of an equation of state denoted by eos in this paper. This equation of state is function of the material density $\rho$ and the material specific internal energy $e = E - \tfrac 1 2 u^2$ as shown in \eqt{eq:EOS}. The equation of state is also used to compute the material temperature $T$ that is required in the relaxation source term $\sigma_a c \left( a T^4 - \epsilon \right)$, where $c$ and $\sigma_a$ denote the speed of light and the absorption cross section, respectively, and $a$ is the radiation constant. The radiation energy density equation, \eqt{eq:GRHrad}, contains a diffusion term function of the total cross section $\sigma_t$. Note that the cross sections $\sigma_a$ and $\sigma_t$ can be either constant or function of the material properties, typically the material density and temperature. Even if all of the results presented and derived in this paper and in \cite{our_jcp_radhy_paper} are independent of the equation of state (as long as the equation of state yields a concave entropy with respect to $e$ and $1/ \rho$), the Ideal Gas equation of state (IGEOS) is considered in the remaining of this paper for simplicity, with the relations given in \eqt{eq:IGEOS} for the material pressure and temperature variables:
%
...
When considering the GRH without the relaxation and the diffusion terms, a first-order partial differential system of equations is obtained that is showed to be hyperbolic: it admits four eigenvalues that are unconditionally real as long as the square of the speed of sound, $c$, remains positive ($\lambda_1 = u - c$, $\lambda_{2,3} = u$ and  $\lambda_4 = u + c$), and has a set of four linearly independent eigenvectors (REF).  

From \eqt{eq:GRH} and assuming a smooth solution i.e. without shocks, an entropy residual equation can be derived by considering an entropy variable function of the material density, the material specific internal energy and the radiation density energy $s(\rho, e, \epsilon)$. The steps leading to the final result given in \eqt{eq:GRH-entropy} can be found in \cite{our_jcp_radhy_paper}.
%
\begin{equation}\label{eq:GRH-entropy}
R_e(x,t) = \frac{D s}{Dt} = \left( \partial_t s + u \partial_x s \right) = 0
\end{equation}
%
We emphasize that \eqt{eq:GRH-entropy} is only valid for smooth numerical solutions and that a different version will be presented in \sct{sec:VR_old} for numerical solutions with discontinuities. Also, in the remaining of this paper, the lefthand-side of \eqt{sec:VR_old}, $R_e$, will be referred to as the \emph{entropy residual}.
...
%
%------------------------------------------------------------------
\subsection{Weak and entropy solutions of non-conservative hyperbolic system of equations}\label{sec:NCSE-theorems}
%------------------------------------------------------------------
%
This section is dedicated to recall some important features of non-conservative hyperbolic systems (NCSE) that are useful to the focus of this paper. As it was previously mentioned in \sct{sec:intro}, the notion of weak and entropy solutions cannot be defined in a distribution sense for NCSE as it is done for conservative systems. The approach of interest for this paper is the one developed by Bianchini and Bressan (REF) that modifies the NCSE by adding a parabolic regularization with a vanishing viscosity coefficient. In Theorem 1 on page 229 of REF, it is stated the Cauchy problem of a non-conservative systems with parabolic regularization admits an unique vanishing viscosity solution.
%
%------------------------------------------------------------------
...
The crue of the method consists in adding dissipative terms to each equation to prevent oscillatory behaviors in the numerical solution. The presence of these additional terms will modify the entropy residual equation (\eqt{eq:GRH-entropy}) for which we must ensure that the entropy condition or entropy minimum principle holds: in other words, the entropy condition is used as a condition to derive a functional form of the additional dissipative terms. As stated before, an earlier demonstration in \cite{our_jcp_radhy_paper} omitted the diffusion and relaxation terms present in \eqt{eq:GRH} and thus, only considered the hyperbolic parts of \eqt{eq:GRH}. We briefly reviews these prior results here that will be later used in \sct{sec:VR_new}. 
...
\end{subequations}
%
where $\kappa$ is a positive and locally defined viscosity coefficient that will be later defined in \sct{sec:def-visc-coeff}. The functional form of the dissipative viscous fluxes was informed from an entropy condition using positivity of the entropy residual $R_e(x,t)$. We provide below the main steps that lead to the derivation of the entropy residual as initially presented in \cite{our_jcp_radhy_paper}. The same procedure will be followed in \sct{sec:VR_new} when diffusion and relaxation terms are incorporated back in the system of equations. Some of the algebraic manipulation may be lengthy and only the final results are provided but no information has been withheld such that the interested reader should be able to reproduce these results. 

As stated in \sct{sec:GRH-inviscid}, the entropy function $s$ for the GRH system is assumed to depend upon density $\rho$, internal energy $e$, and radiation energy $\epsilon$, i.e., $s( \rho, e, \epsilon)$. Invoking the chain rule
%
\begin{equation}
\label{eq:app_equ2}
...
the specific kinetic energy equation is simply obtained by multiplying by $u$ the momentum equation expressed in terms of primitive variables. After some algebra and the use of \eqt{eq:app_equ2}, the following entropy residual equation is obtained:
%
...
The entropy function $s_{Euler}(\rho, e)$ is concave with respect to the internal energy $e$ and the specific volume $\rho^{-1}$ and is defined through the second law of thermodynamics recalled in \eqt{eq:scn-law-th}.
%
\begin{equation}\label{eq:scn-law-th}
Tds_{Euler} = de + P d \rho^{-1}
\end{equation}
%
The entropy function $s_{rad}(\rho,\epsilon)$ is concave with respect to the radiation energy density $\epsilon$. Positivity of the last term, $s_e \rho \kappa (\partial_x u)^2$, in the righthand-side of \eqt{eq:app_entr_eq_non_equil} depends on the sign of $s_e$ that can be determined in two simple steps: first, using the functional form given in \eqt{eq:ent_equ} we devise $s_e = (s_{Euler})_e$. Then, using the previous result and \eqt{eq:scn-law-th}, we infer that $s_e = T^{-1} \leq 0$. Recalling that the material density $\rho$ and the viscosity coefficient $\kappa$ are positive, positivity of the term $s_e \rho \kappa (\partial_x u)^2$ is now trivial.
%
...
%
We now propose to investigate in \sct{sec:VR_new} the minimum entropy principle for the full GRH equations, i.e. when accounting for the diffusion term, $\partial_x \left( \frac{c}{3 \sigma_t} \partial_x \epsilon \right)$, and the relaxation source terms, $\sigma_a c \left( a T^4 - \epsilon \right)$.
...
\end{equation}
%
and thus combine \eqt{eq:GRHmass_}, \eqt{eq:GRHenerg_}, and \eqt{eq:GRHrad_} using the chain rule given in \eqt{eq:app_equ2} to obtain
%
...
\end{equation}
%
If we can prove that the right-hand side of \eqt{eq:entro_eq_all_terms} is unconditionally positive, then the proposed viscous regularization, shown in the hyperbolic portion of the GRH equations in \eqt{eq:regularized_hyperbolic_GRH}, will
also be entropy-minimum satisfying for the full GRH equations. 

The right-hand side of \eqt{eq:entro_eq_all_terms} contains two terms which we analyze separately. Using the entropy function given in \eqt{eq:ent_equ}, the first term in the righthand-side becomes
%
\begin{equation} 
...
\end{equation}
%
with $x=  \left(\frac{aT^4}{\epsilon}\right)^\frac{1}{4} \geq 0$ and remembering that the speed of light, $c$ and the absorption opacity, $\sigma_a$ are positive by definition. \tcb{What about $\epsilon$?}.
