...
for solving radiation hydrodynamic shock and equilibrium-diffusion limit simulations. 
...
Radiative shock calculations using constant and temperature-dependent opacities are presented for various Mach numbers and 
compared with semi-analytical reference solutions. \tcb{Convergence rates are provided.}
\end{abstract}
%-------------------------
...
according to the MultiMat-2015 email.} \tcb{Is not it the correct template?}
...
The GRH equations form a non-conservative multi-physics system of equations and couple the inviscid 
...
that contain neither spatial nor temporal derivative terms, and yield the following behavior: when the material is optically thick, i.e., 
...
3 tests with constant and temperature-dependent opacity are given in \sct{sec:num-rslt} and compared against semi-analytical solutions obtained from the method detailed in \cite{jim_ferguson} \tcb{I think we should use Jim's dissertation for the reference. I emailed him for a more specific paper}. Test cases with 
...
(as long as it admit a concave entropy with respect to $e$ and $1/ \rho$). However, the Ideal Gas Equations of State
...
of equations is obtained; this system is hyperbolic, admits four eigenvalues that are unconditionally real as long as the square of the speed of sound, $c_m$, remains positive ($\lambda_1 = u - c_m$, $\lambda_{2,3} = u$ and  $\lambda_4 = u + c_m$), and possesses a set of four linearly independent eigenvectors (REF; the first-order partial differential system of equations is thus hyperbolic.  
