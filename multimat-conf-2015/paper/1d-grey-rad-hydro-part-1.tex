...
\tcr{and recover the EDL equations ... write something more, show the VR for EDL equations?}


The entropy viscosity method is now well defined for the hyperbolic system given in \eqt{eq:regularized_hyperbolic_GRH} and will be used to solve for the non-equilibrium Grey Radiation-Hydrodynamic equations given in \eqt{eq:GRH}. \tcr{this makes no sense for THIS paper. Needs updating}.
...
\end{eqnarray*}
%
\tcr{there is no $\mu$ and thus $H$ can be much simpler}
...
An implicit second-order BDF2 temporal scheme is employed \cite{Leveque} \tcr{Leveque as a ref for BDF2???}. 
...
\tcr{pressure} boundary condition is implemented (the back pressure is specified and the other hydrodynamic variables are computed from the characteristics). 
