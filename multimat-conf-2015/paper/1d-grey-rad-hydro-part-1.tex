...
\runningheads{M.~O.~Delchini, J.~C.~Ragusa}{Viscous Regularization of the non-equilibrium Grey Radiation-Hydrodynamics}
...
%-------------------------
\begin{abstract}
\tcb{did we have Dr Morel as a third author on the paper? I do not mind having him as a third author but just wanna be consistent.}
In \cite{our_jcp_radhy_paper}, 
a viscous regularization technique based on the local entropy residual was
developed to stabilize numerical solutions to the non-equilibrium Grey 
...
In this method, the artificial viscosity coefficient is modulated by the local entropy production and peaks at shock locations. 
...
(i) because of the wave-dominated nature of the GRH equations, \emph{a numerical method} is required to preserve the 
solution's wave structure; 
(ii) when only considering the first-order hyperbolic terms of the GRH equations for the development of a numerical method, the resulting system equations cannot be written in a conservative form and thus, the notions of weak and entropy 
...
(iii) finally, the \emph{numerical scheme} devised needs to preserve the equilibrium-diffusion limit. For an artificial viscosity technique,
this means that the viscous regularization dissipative terms must be well-scaled in such a limit, i.e., that the regularized GRH equations 
deveolve to the EDL system of equations.
%
...
As highlighted above, the theory developed by DLM \cite{dlm} for non-conservative hyperbolic system of equations of the form given in \eqt{eq:nc-syst-eq} is needed, in order to define in a weak sense the non-conservative products $A(U) \partial_x U$:
%
...
written in a conservative form, i.e., matrix $A(U)$ is not the jacobian matrix of vector-valued function $f: 
\mathbb{R}^n \to \mathbb{R}^n$ such as $\frac{\partial f(U)}{\partial U} \ne A(U)$, and 
Systems of equations of the form given by  \eqt{eq:nc-syst-eq} are often encountered in two-phase flow models 
\cite{Saurel_2009, Ambroso_2012, Zein_2010, Li_2004, Saurel_2001b, Saurel_2001a, GuillardMurrone2003}, for instance. With the above assumptions, 
the non-conservative product $A(U) \partial_x U$ is not defined in the sense of distributions, % when considering a Riemann problem, 
that is, one cannot rigorously define the notions of weak and entropy solutions as it is typically done for 
...
and the notion of shock, rarefaction, and contact waves for non-conservatie systems. The DLM theory also applies to conservative systems of 
...
\tcr{that paragraph about the DLM is super long and then, here, we seem to say we switch to something else. The question for a reader would be: why?}
...
%
This generalization of the entropy inequality to non-conservative hyperbolic systems of equations is of interest 
for the topic of this paper: in \cite{our_jcp_radhy_paper}, Delchini et al. proposed to solve the non-equilibrium Grey Radiation-Hydrodynamics 
...
results for constant and temperature-dependent opacities and provide convergence rates \tcb{not yet} using semi-analytical solutions.
...
principle when one considers the full GRH equations, i.e. with relaxation and diffusion terms.
After describing discretization and solution techniques in \sct{sec:discr}, 1-D numerical results of Mach 
3 tests with constant and temperature-dependent opacity are given in \sct{sec:num-rslt} and compared against semi-analytical solutions.
\tcr{no need for this here. This is too much details. Ferguson should be cited int he results sections, when we say again that we are using semi-analytical solutions}. 
...
an equation of state, shown as $eos$ in \eqt{eq:EOS}, relates the material peressure $P$ to the material density $\rho$ and the 
...
When omitting the relaxation and the diffusion terms from the GRH equations, i.e., the righthand side of \eqt{eq:GRH} is zeroed out, 
...
speed of sound, $c_m$, remains positive ($\lambda_1 = u - c_m$, $\lambda_{2,3} = u$ and  $\lambda_4 = u + c_m$), and possesses a set of 
four linearly independent eigenvectors (REF).  
...
Assuming a smooth solution and the existence of an entropy function $s=s(\rho, e, \epsilon)$, the following entropy residual equation can be derived from the first-order hyperbolic system of equations, i.e. the GRH given in \eqt{eq:GRH} without the relaxation and diffusion terms:
...
Hereafter, the term $R_e$ in the left-hand-side of \eqt{sec:VR_old}, will be referred to as the \emph{entropy residual}.
...
the notion of weak and entropy solutions cannot be defined in a distribution sense for NCSE. Instead, we are using in this paper the theoretical results by 
...
As previously defined in \sct{sec:GRH-inviscid}, the entropy function $s$ for the GRH system is assumed to depend upon the density $\rho$, the internal energy $e$, and the radiation energy $\epsilon$, i.e., $s( \rho, e, \epsilon)$. Invoking chain rule, we can write
%
\begin{equation}
...
\end{equation}
which demonstrates that the entropy equation of the full GRH equations satisfies an inequality and is positive. Consequently, in order to prove that the entropy residual $R_e(x,t)$ is positive which is required to satisfy an entropy minimum principle, positivity of the density is needed. Guermond et al. proved in Lemma 3.1 of \cite{jlg} positivity of the material density $\rho$ when the continuity equation, \eqt{eq:GRHmass}, is regularized by a viscous term of the form $\partial_x ( \kappa \partial_x \rho )$. Thus, because the continuity equation of the GRH is regularized of the same way as shown in \eqt{eq:cont_eq_reg}, we infer that positivity of the material density is ensured. We have proved:
%
\begin{theorem}[Entropy-minimum principle for the full GRH]
...
definition of the numerical scheme. Various concepts may be invoked to define the viscosity coefficient $\kappa$ as it is only required to be positive for Theorem 3.1 to apply. The guiding principle
...
to the idea of bounding (from above) the entropy viscosity with a first-order viscosity (e.g., a local Lax-Friedrichs viscosity \tcb{or Godunov?}).
...
\end{subequations}
%
where $\hat{E} = E + \frac{T^4}{\rho} $ is the radiation-modified specific energy whom definition matches the definition given in \cite{LowrieMorel}. By setting the high-order viscosity coefficient proportional to the local entropy residual and jumps, we ensure correct behavior of the numerical method when dealing with temperature-dependent opacity \tcb{This is analogous to transonic flows}.
...
%------------------------------------------------------------------
%
We present 1-D numerical results for Mach 3 radiation hydrodynamic simulations in a pure absorber material: the first test corresponds to a material with constant cross sections (\sct{sec:mach-3-cst-xs}), and the second test corresponds to a material with temperature dependent opacities (\sct{sec:mach-3-no-cst-xs}). BDF2 time integration, linear continuous finite elements, and an uniform grid of $1000$ cells is used to discretize the problem. A CFL number of 10 is 
...
\mathbb{\Sigma} =  \frac{\sigma_{a, \infty} L_\infty c}{c_{m,\infty}} \text{ (Absorption cross section) } \ ,
...
%\end{table}
%
For this test, the inlet Mach number is set to $3$ and the total and absorption cross sections are assumed constant. The initial conditions are computed from the four non-dimensional numbers $\mathbb{P}_\infty=10^{-4}$, $M=3$, $\mathbb{\Sigma}=10^6$ and $\mathbb{\Lambda}=1$ which yields a cross section of $577.5 \ cm^{-1}$ \tcb{value to check}. The steady-test numerical results of the material density. the material and radiation temperatures, and the viscosity coefficient $\kappa$ are presented in \fig{fig:mach-3-cst-xs-dens}, \fig{fig:mach-3-cst-xs-temp} and \fig{fig:mach-3-cst-xs-visc}, respectively, along with the semi-analytical solutions when available.
\begin{figure}[H]
    \centering
...
\end{figure}
%
The viscosity coefficients $\kappa$ displays a peak in the vicinity of the shock around $x=0 \ cm$ as shown in \fig{fig:mach-3-cst-xs-visc}: this behavior is conformed to the definition of the viscosity coefficient $\kappa$ that is set proportional to the local entropy residual and the jumps (see \eqt{eq:equation12}). It is noticed, however, that the viscosity coefficient $\kappa$ does not saturate to the over-dissipative viscosity coefficient $\kappa_{max}$ but still efficiently stabilizes the numerical solution. An explanation to this observation holds
in the dissipative behavior of the source terms as they are entropy productive and thus add dissipation, as proved in \sct{sec:VR_new}.
%
%------------------------------------------------------------------
...
We now consider a Mach 3 test in a pure absorber material with the same non-dimensional numbers as in \sct{sec:mach-3-cst-xs}. The cross sections are no longer constant but depend on the material properties, i.e the density $\rho$ and the material temperature $T$. Expression of the \emph{non-dimensional} form of the temperature-dependent cross section, $\tilde{\sigma}$, is given in \eqt{eq:opacity}:
%
\begin{equation}\label{eq:opacity}
\tilde{\sigma}_a = \tilde{\sigma}_t = \tilde{\rho} \frac{12.5}{\tilde{T}^{\frac{2}{5}}}\, ,
\end{equation}
%
where the upper-script '$\ \tilde{} \ $' denotes non-dimensional variables. The dimensional cross sections can be obtained from \eqt{eq:opacity} by multiplying by the reference cross section $\sigma_{a, \infty} = \sigma_{t, \infty}$ computed from the non-dimensional numbers given in \eqt{eq:scaled-nb}.

This test aims at testing the entropy viscosity method with a non-constant cross section. From a theoretical perspective as proved in \sct{sec:def-visc-coeff}, the entropy inequality still holds, meaning that the viscous regularization should efficiently stabilize any discontinuity or shock that would from in the numerical solution. 

The test is run until steady state is reached. Numerical results of the material density and, the material and radiation temperatures are presented and plotted against semi-analytical solutions in \fig{fig:mach-3-dpt-xs-dens} and \fig{fig:mach-3-dpt-xs-temp}, respectively. The profile of the total/absorption cross section is given in \eqt{fig:mach-3-dpt-xs-xs}. We also plot the viscosity coefficient profiles at steady state in \fig{fig:mach-3-dpt-xs-visc}.
%\begin{itemize}
...
\end{figure}
%
The density profile plotted in \fig{fig:mach-3-dpt-xs-dens} does not show any instability neither in the vicinity of the shock around $x = 0 \ cm$, nor in the abrupt change located at $x = -0.01 \ cm$. The shock is well resolved and the numerical and semi-analytical solutions overlap.
%
\begin{figure}[H]
    \centering
...
\end{figure}
%
\tcb{Plot the semi-analytical solution that can be computed from the density and temperature semi-analytical solutions}. \\ 
The numerical profile of the cross section is plotted in \fig{fig:mach-3-dpt-xs-xs} against the semi-analytical solution that was computed from the semi-analytical solutions of the material density and temperature. We recall here that the material is a pure absorber and thus, the total and absorption cross sections are equal. The cross section varies by a factor $20$ in the shock region which .... \tcb{Are we not in the case where the material is optically thick in the pre-shock region? If this is the case, we should write something about the interest of using the local entropy residual with a local normalization as we do for Euler equation.}
%
\begin{figure}[H]
    \centering
...
The profiles of the high- ($\kappa_e$) and low-order ($\kappa_{max}$) viscosity coefficients are showed in \fig{fig:mach-3-dpt-xs-visc}. It is first observed that the high-order viscosity coefficient $\kappa_e$ displays two peaks at $x=-0.01 \ cm$ and at $x = 0 \ cm$. The latter one coincides with the shock position where the entropy residual $R_e(x,t)$ is known to be peaked (see \sct{sec:def-visc-coeff} and also \cite{our_jcp_radhy_paper}). The former one, however, is due to the inclusion of the jumps in the definition of the high-order viscosity coefficient as shown in \eqt{eq:equation12} and \eqt{eq:equation13}: the material density does not experience a shock at $x=-0.01 \ cm$ but a sharp variation that triggers the jump. It is also observed that the high-order viscosity coefficient $\kappa_e$ does not saturate to the low-order viscosity coefficient $\kappa_{max}$ in the vicinity of the shock around $x = 0 \ cm$ which can be explained by the dissipative behavior of the source terms that add dissipation as highlighted in \sct{sec:VR_new}. Overall, the high-order viscosity coefficient behaves as expected: it is peaked in the vicinity of the shock and in region of sharp variations, and small elsewhere.
%
%------------------------------------------------------------------
%------------------------------------------------------------------
...
%------------------------------------------------------------------
%------------------------------------------------------------------
In this paper, the theoretical ground for the application of the entropy viscosity method to the full non-equilibrium Grey Radiation Hydrodynamic equations, i.e. with source term, has been presented. We first highlighted the notions of weak solution and entropy condition for non-conservative hyperbolic system of equation in order to introduce our logic. Then, we demonstrate that with the formal form of the entropy function derived in \cite{our_jcp_radhy_paper}, (i) an entropy condition still holds when accounting for the relaxation and diffusion terms, (ii) the same viscous regularization can be employed to stabilize the GRH equations and, (ii) the asymptotic study of the GRH equations is still equivalent to the equilibrium-diffusion limit (EDL) equations.

Using the above theoretical results, the definition of the low- and high-order viscosity coefficients derived in \cite{our_jcp_radhy_paper} are shown to be still valid. We then presented two Mach 3 numerical results of the 1-D GRH equation with constant and material-dependent cross sections. It was observed that the entropy viscosity method behaves very satisfactorily. Physical features such as embedded hydrodynamic shocks and the Zeldovich spike are resolved accurately without spurious oscillations. The entropy viscosity coefficient is only peaked in the shock region and remains small elsewhere, as expected.
\\ \tcb{future work? Not obvious too me besides what you already mentioned in \cite{our_jcp_radhy_paper}.}
%
%\section{Introduction}
%\label{sec:section1}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
