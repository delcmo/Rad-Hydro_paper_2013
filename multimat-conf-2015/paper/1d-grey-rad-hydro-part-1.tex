...
\section{The non-equilibrium grey Radiation-Hydrodynamic equations}
Give the system of equations and define the variables.
\section{A viscous regularization for the GRH based on their hyperbolic parts (REF)}
Recall the previous results.
\section{Entropy minimum principle for the complete GRH equations}
New parts: show that the entropy minimum principle is still verified when using the functional form of the entropy derived in the previous section.
\section{1-D Radiation-Hydrodynamic shock results}
\subsection{An entropy-based definition of the viscosity coefficients (REF)}
