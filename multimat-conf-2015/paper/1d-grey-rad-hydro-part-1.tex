...
%------------------------------------------------------------------
%
We present 1-D numerical results for Mach 3 radiation hydrodynamic simulations with a pure absorber material: the first test corresponds to a material with constant cross sections (\sct{sec:mach-3-cst-xs}), and the second test corresponds to a material with temperature dependent opacities (\sct{sec:mach-3-no-cst-xs}). BDF2 time integration, linear continuous finite elements, and an uniform grid of $500$ cells is used to discretize the problem. A CFL number of 10 is 
...
For each numerical results presented in \sct{sec:mach-3-cst-xs} and \sct{sec:mach-3-no-cst-xs}, the initial conditions are computed from four non-dimensional numbers that are defined below in \eqt{eq:scaled-nb} where the symbol $\infty$ denotes the reference values (see Section 2 of \cite{LowrieEdwards} for further details).
%
...
\end{eqnarray}
%
The values of the above non-dimensional numbers will be specified for each numerical tests presented in this paper. We recall that in the case of a pure-absorber material, the total and absorption cross sections are set equal.
%
%------------------------------------------------------------------
...
\end{figure}
%
The viscosity coefficients $\kappa$ displays a peak in the vicinity of the shock around $x=0 \ cm$ as shown in \fig{fig:mach-3-cst-xs-visc}: this behavior is conformed to the definition of the viscosity coefficient $\kappa$ that is set proportional to the entropy residual (see \eqt{eq:equation12}). It is noticed, however, that the viscosity coefficient $\kappa$ does not saturate to the over-dissipative viscosity coefficient $\kappa_{max}$ but still manages to efficiently stabilize the numerical solution. An explanation to this observation holds
in the dissipative behavior of the source terms as they are entropy productive, as proved in \sct{sec:VR_new}.
%
%------------------------------------------------------------------
