...
\tcr{utilized in the context of discontinuous spatial discretization techniques} \tcb{I think it is good to emphasize this}. In \cite{Balsara}, an 
...
solution at points of discontinuity so that the non-conservative product admits a weak solution. \tcb{The Borel measure of 
...
%\end{table}
%
For this test, the inlet Mach number is set to $3$ and the total and absorption cross sections are assumed constant. The initial conditions are computed from the four non-dimensional numbers $\mathbb{P}_\infty=10^{-4}$, $M=3$, $\mathbb{\Sigma}=10^6$ and $\mathbb{\Lambda}=1$ which yields a cross section of $577.35 \ cm^{-1}$. The steady-test numerical results of the material density. the material and radiation temperatures, and the viscosity coefficient $\kappa$ are presented in \fig{fig:mach-3-cst-xs-dens}, \fig{fig:mach-3-cst-xs-temp} and \fig{fig:mach-3-cst-xs-visc}, respectively, along with the semi-analytical solutions when available.
\begin{figure}[H]
    \centering
...
We now consider a Mach 3 test in a pure absorber material with the same non-dimensional numbers as in \sct{sec:mach-3-cst-xs}. The cross sections are no longer constant but depend on the material properties, i.e the density $\rho$ and the material temperature $T$. Expression of the \emph{dimensional} form of the density- and  temperature-dependent cross sections, $\tilde{\sigma}$, is given in \eqt{eq:opacity}:
%
...
This test aims at testing the entropy viscosity method with a cross section that experiences strong variation in the shock region. From a theoretical perspective as proved in \sct{sec:def-visc-coeff}, the entropy inequality still holds, meaning that the viscous regularization should efficiently stabilize any discontinuity or shock that would from in the numerical solution. 
