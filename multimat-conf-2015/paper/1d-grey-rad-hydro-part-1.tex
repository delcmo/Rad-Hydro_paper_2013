...
\runningheads{M.~O.~Delchini, J.~C.~Ragusa, J.~E.~Morel}{Viscous Regularization of the non-equilibrium Grey Radiation-Hydrodynamics}
...
developed to stabilize numerical solutions \tcr{to} the non-equilibrium Grey Radiation-Hydrodynamic equations. 
...
entropy viscosity method ; viscous stabilization method ; equilibrium-diffusion limit.}
...
%------------------------------------------------------------------
%------------------------------------------------------------------
Accurately solving \tcr{for} the non-equilibrium Grey diffusion Radiation-Hydrodynamic (GRH) equations with 
...
\begin{itemize}
\item
(i) because of the wave-dominated nature of the GRH equations, the solution technique should preserve the wave structure; 
\item
(ii) when only considering the hyperbolic terms of the GRH equations for the development of a numerical method \tcr{because this is no longer true for the full GRH equations???}, the resulting system equations cannot be written in a conservative form and thus, the notions of weak and entropy 
...
\item
(iii) finally, the numerical scheme devised needs to preserve the equilibrium-diffusion limit (EDL). For an artificial viscosity technique,
this means that the dissipative terms must scale appropriately in such a limit, i.e., that the regularized GRH equations 
devolve to the EDL system of equations.
\end{itemize}
%
...
\tcr{utilized in the context of discontinuous spatial discretization techniques}. In \cite{Balsara}, an 
...
The aim of this paper is \tcr{three}-fold. 
...
Second, we present numerical results for constant and temperature-dependent opacities and provide convergence rates 
\tcb{not yet} using semi-analytical solutions.
...
The viscous regularization of the GRH equations {\it based on their hyperbolic parts} is recalled in \sct{sec:VR_old}.
In \sct{sec:VR_new}, we demonstrate how that the previous viscous regularization still satisfies the entropy minimum 
principle when one considers the {\it full} GRH equations, i.e., with relaxation and diffusion terms present. We also
show that \tcb{add something in the introduction about the ED limit.}
...
\end{subequations}
%
where $\rho$, $\rho u$ and $\rho E$, $P$, $T$ are the material density, material momentum, material total energy, material pressure and 
material temperature, respectively, and $\epsilon$ denotes the radiation energy density.  
...
an equation of state that relates the material pressure $P$ to the material density $\rho$ and the 
...
$c$ and $\ar$ are denote the speed of light and the radiation constant; $\sigma_a$ is absorption opacity. 
...
material properties (material density and temperature). 
...
When omitting the relaxation and the diffusion terms from the GRH equations, i.e., the right-hand side of \eqt{eq:GRH} is zeroed out, 
