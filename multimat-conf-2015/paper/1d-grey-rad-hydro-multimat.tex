...
compared with semi-analytical reference solutions by performing a convergence study. % \tcr{Convergence rates are provided. TBD}
\end{abstract}
%-------------------------
...
\end{enumerate}
%
Significant research efforts regarding the GRH equations have been devoted to the elaboration of 
...
written in a conservative form, i.e., matrix $A(U)$ is not the jacobian matrix of a vector-valued function $f(U): 
...
The new theoretical results are illustrated in \sct{sec:num-rslt} with numerical results for Mach 1.05 and Mach 3 tests. For each test, the numerical solution is compared against semi-analytical solutions by performing a convergence study. 
...
%------------------------------------------------------------------
%
The definition of the  entropy viscosity coefficient $\kappa$ is recalled below (we refer the reader to Section 2.4 of \cite{our_jcp_radhy_paper} for further details). Through the entropy residual $R_e$ of \eqt{eq:app_entr_eq_non_equil} can be recast
as a function of total pressure (the sum of fluid and radiation pressures) and density, $\hat R_e = \matder{\left(P+\tfrac 1 3 \epsilon\right)} - c^2_m \matder{\rho}$. An asymptotic limit study in the EDL reveals that a proper normalization for this residual is $\rho c^2_m$. Then, the entropy viscosity is computed as
\begin{equation}
\kappa_e = h^2 \frac{|\hat R_e|}{\rho c^2_m}
\end{equation}
The entropy viscosity is bounded from above by a first-order viscosity (Rusanov's):
\begin{equation}
\kappa_\text{max} = \tfrac 1 2 \left( |u| + c_m \right)
\end{equation}
Finally, the entropy coefficient coefficient is given by the minimum of the entropy viscosity and the first-order viscosity:
\begin{equation}
\kappa = \min \left( \kappa_e, \kappa_\text{max} \right)
\end{equation}

\tcb{Should we add a few words about $\kappa_{max}$ and $\kappa_e$ along with equation numbers from previous paper?}
\tcr{we refer a whole lot tot he other paper. maybe a very short and sweet paragraph on the coefficients would be better. I tried. see what you think. remark: in our first paper, I noted that the entropy residual, verylast eq of the appendix, can also just be given in terms of fluid variables, P fluid, rho fluid, c fluid. looks like we don't need to care about radiation at all in this model...}
...
the entropy equation obtained for the full GRH and recalled in \eqt{eq:app_entr_eq_non_equil}, devolves to the EDL entropy equation given in \eqt{eq:EDL-entropy}. Such a behavior was assumed but remained to be proved. These two mathematical results further warrant an entropy viscosity-based approach for the stabilization of the GRH equations 
and help in understanding  some of the features observed in the numerical results presented in \sct{sec:num-rslt} and in \cite{our_jcp_radhy_paper}. \\
...
\tcr{check my mods in this entire paragraph}
%
%------------------------------------------------------------------
...
%
Let is denote by $P$ the Jacobian matrix associated to the change of variable proposed in \eqt{eq:chge_var}, i.e., $X=PY$. Then, the quadratic form in the new basis is obtained as follows:
%
\begin{equation}
...
For the hydrodynamics equations, implementation of the boundary conditions depends upon the value of the Mach number (supersonic and subsonic). For supersonic inlet flows, Dirichlet boundary conditions are used
since no physical information exits the system. On the other hand, at the outlet the flow becomes subsonic and thus requires a more sophisticated implementation: a static 
pressure boundary condition is weakly imposed by specifying a back pressure denoted by $P_b$, and by computing the other hydrodynamic variables from the relevant characteristics (\tcr{REF}). 
For the diffusion term of the radiation diffusion equation (\eqt{eq:GRHrad}), a vacuum boundary condition is used at both inlet and outlet. 

For each numerical results presented in \sct{sec:mach-1p05-cst-xs}, \sct{sec:mach-3-cst-xs} and \sct{sec:mach-3-no-cst-xs}, 
the initial conditions consist of a step located at $x=0 \ cm$ with constant left and right values computed from \eqt{eq:scaled-nb} and the Rankine-Hugoniot condition (see Section 2 of \cite{LowrieEdwards}).
...
The symbol $\infty$ denotes the reference values in \eqt{eq:scaled-nb} (see Section 2 of \cite{LowrieEdwards} for further details).
%
...
\end{eqnarray}
%
We recall that, in the case of a pure-absorber material, the total and absorption opacities are set equal.

The remaining of this section is organized as follows: in \sct{sec:mthd-conv-test} the method employed to perform the convergence study and to compute the $L_1$ and $L_2$ norms of the error between the numerical solutions and the semi-analytical solutions are detailed. Then, the numerical results and the convergence studies are presented in \sct{sec:mach-1p05-cst-xs}, \sct{sec:mach-3-cst-xs} and \sct{sec:mach-3-no-cst-xs} for Mach 1.05 and Mach 3 with constant opacity, and Mach 3 with temperature-dependent opacities, respectively. For each test case, information regarding the CFL condition and the number of elements used in the mesh are provided. Steady-state numerical results for material density $\rho$ and temperature $T$,  radiation temperature $T_r$, and Mach number $Mach$ are shown and compared against semi-analytical solutions. The profiles for viscosity coefficients $\kappa$ and $\kappa_{max}$ are also graphed to illustrate the behavior of the EVM.
%
%------------------------------------------------------------------
...
Because the semi-analytical solution is Galilean invariant \tcb{Jim, is this correct?}, the semi-analytical solution needs to be shifted to overlap with the numerical solution when performing a convergence study. This shifting can be achieved by conserving either mass, momentum or total energy ($(\rho E)_{tot}= \rho E + \epsilon$) that correspond to the conserved variables in the GRH. At steady state, from the mass equation, it is deducted that momentum is constant and thus cannot be used to compute a unique shift value. We are thus left with two options that are conservation of mass and total energy. In the following, only the shifting procedure using mass conservation is presented and is easily extended to the total energy conservation case.
%use conservation of mass or total energy (material energy plus radiation energy) that are conserved to minimize 
The objective of the method is to minimize the mass difference between the semi-analytical and the numerical solutions, $m_{anal}$ and $m_{num}$, respectively. The mass difference is computed by integrating the density, a conservative variable, over a domain $[x_1, x_2]$ common to the numerical and the semi-analytical solution and containing the shock when present, as shown in \eqt{eq:mass-diff}:
%
...
\end{equation}
%
where $x_{offset}$ is an offset value that is used to shift the semi-analytical solution towards the numerical solution. Once the absolute value of the mass difference becomes smaller that a given tolerance, say $10^{-10}$, the value of $x_{offset}$ is retrieved. Then, the $L_{1,2}$ error norms of a variable of interest denoted by $y(x)$, is computed as follows:
%
\begin{eqnarray}\label{eq:l1-error-norm}
L_{1,2}(y) &=& \left[ \int_{x_1}^{x_2} | y_{anal}(x+x_{offset}) - y_{num}(x) |^{1,2} dx \right]^{1,1/2} \\ 
%&=& \left[ \sum_{i=1}^{nb_{cells}} \int_{x_i}^{x_{i+1}} | y_{anal}(x+x_{offset}) - y_{num}(x) |^{1,2} dx \right]^{1,1/2} \\ 
&=& \left[ \sum_{i=1}^{nb_{cells}} \sum_{x_q} | y_{anal}(x_q+x_{offset}) - y_{num}(x_q) |^{1,2} dx \right]^{1,1/2} \, .
\end{eqnarray}
%
The integral over the computational domain delimited by $[x_1; x_2]$ is split over $nb_{cells}$ cell integrals. Each cell integral is numerically computed with a quadrature rule (Gauss quadrature rule of order $10$) that requires to evaluate the semi-analytical and the numerical solutions at the quadrature points. The numerical solution is evaluated at the quadrature points with the finite element approach using Lagrange test functions. On the other hand, the values of the semi-analytical solution at the quadrature points are computed with a linear interpolation (Fortran version of the function '\emph{interp1d}'). A typical semi-analytical solution with shock is solved on a non-uniform mesh discretized with $10^4$ nodes (to compare to $1000$ nodes for the numerical solution), which should minimize the error due to the interpolation. It is chosen to shift the semi-analytical solution since it is solved on a finer mesh than the numerical solution. The exact same procedure can be achieved for conservation of total energy when replacing $m_{anal}$ and $m_{num}$ with $\rho E_{anal}$ and $\rho E_{num}$, respectively, in \eqt{eq:mass-diff}.

A spatial convergence study is performed for each test case presented in this paper, which requires to run multiple times the same simulation on successive finer mesh. For each new run, once the steady-state solution is obtained, a value of the shift, $x_{offset}$, is computed by minimizing either the the mass or the total energy difference, and further used to compute the $L_1$ and/or $L_2$ error norm given in \eqt{eq:l1-error-norm}.
%
%------------------------------------------------------------------
\subsection{Mach-1.05 test case with constant opacities}\label{sec:mach-1p05-cst-xs}
%------------------------------------------------------------------
%
For this test, the inlet Mach number is set to $1.05$ and the total and absorption cross sections are assumed constant. The initial conditions and opacity are computed from the four non-dimensional numbers $\mathbb{P}_\infty=10^{-4}$, $M=1.05$, $\mathbb{\Sigma}=10^6$ and $\mathbb{\Lambda}=1$ defined in \eqt{eq:scaled-nb}, which yields an opacity of $577.35 \ cm^{-1}$. This test case has multiple purposes. First, it will serve at illustrating further the methodology proposed in \sct{sec:mthd-conv-test} for shifting the semi-analytical solution towards the numerical solution with mass and total energy conservations. Then, since the Mach-1.05 test case corresponds to the EDL and yields a smooth solution, second-order accuracy of the EVM will be demonstrated by performing a convergence study of the $L_1$ and $L_2$ error norm. Lastly, it will also allow to illustrate the mathematical prove presented in \sct{sec:cons-equi-diff-limit} on the consistency of the entropy equation in the EDL.
...
variations of the computed offset value $x_{offset}$ for meshes ranging from $10$ to $250$ cells are presented in \tbl{tbl:mach-1p05-x-offset} when using mass ($\Delta \rho$) and energy ($\Delta (\rho E_{tot})$) conservation, respectively.
...
\end{tabular}
\end{center}
\caption{$x_{offset}$ values computed from mass and energy conversations.}\label{tbl:mach-1p05-x-offset}
\end{table}
%
It is observed that both methods yield close $x_{offset}$ values: $x_{offset}$ decreases as the mesh becomes finer to reach a plateau value of $3.8 \times 10^{-4}$. The numerical and semi-analytical density profiles are shown before and after shifting the semi-analytical solution in \fig{fig:mach-1p05-cst-xs-density-no-shift} and \fig{fig:mach-1p05-cst-xs-density-with-shift}, respectively. The numerical solution was obtained on a computational domain meshed with $100$ cells. By minimizing the mass or total energy difference between the numerical and semi-analytical solutions, the semi-analytical solution is shifted towards the right boundary.
%
\begin{figure}[h]
%    \begin{subfigure}{0.5\textwidth}
%    \centering
%    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mass-diff-mach-1p05-x-offset.eps}
%    \caption{$x_{offset}$ vs. number of cells with mass conservation.}\label{fig:mass-diff-mach-1p05-x-offset}
%    \end{subfigure}
%    \begin{subfigure}{0.5\textwidth}
%    \centering
%    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/energy-diff-mach-1p05-x-offset.eps}
%    \caption{$x_{offset}$ vs. number of cells with total energy conservation.}\label{fig:energy-diff-mach-1p05-x-offset}
%    \end{subfigure}    
    ...
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-density-nel-50.eps}
...
\end{figure}
%
The convergence rates for the material density, the material temperature, the radiation temperature and the mach number are given in \fig{fig:mach-1p05-cst-xs-conv} for the $L_1$ and $L_2$ error norms between the numerical and semi-analytical solutions for the variables $(\rho, T, \epsilon, Mach)$. A reference line of slope $2$ is also plotted which clearly shows that second-order accuracy is achieved for both the $L_1$ and $L_2$ error norms when refining the mesh. This example proves that the EVM achieves second-order accuracy for smooth solutions and that the entropy condition still holds in the EDL as proved in \sct{sec:cons-equi-diff-limit}.
%
\begin{figure}[h]
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mass-diff-mach-1p05-mat-temp-convergence.eps}
    \caption{Material temperature.}\label{fig:mach-1p05-cst-xs-temp-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mass-diff-mach-1p05-density-convergence.eps}
    \caption{Material density.}\label{fig:mach-1p05-cst-xs-density-conv}
    \end{subfigure}
    
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mass-diff-mach-1p05-mach-number-convergence.eps}
    \caption{Mach number.}\label{fig:mach-1p05-cst-xs-mach-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mass-diff-mach-1p05-radiation-convergence.eps}
...
The numerical and semi-analytical solutions of $(\rho, T, T_r, Mach)$ are plotted in \fig{fig:mach-1p05-cst-xs-density}, \fig{fig:mach-1p05-cst-xs-temp}, \fig{fig:mach-1p05-cst-xs-radiation} and \fig{fig:mach-1p05-cst-xs-mach}, respectively. It is observed that the numerical profiles are smooth and perfectly match the semi-analytical solutions for each variable. The viscosity coefficients $\kappa$ and $\kappa_{max}$ are plotted in \fig{fig:mach-1p05-cst-xs-visc} on a log scale. The entropy viscosity coefficient $\kappa$ is three to seven order of magnitude smaller than the first-order viscosity coefficient $\kappa_{max}$. Such behavior is expected as the numerical solution is smooth and the entropy production small.
%
\begin{figure}[h]
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-mat-temp-nel-250-plot.eps}
    \caption{Material temperature.}\label{fig:mach-1p05-cst-xs-temp}
    \end{subfigure}
    ~
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-density-nel-250-plot.eps}
    \caption{Material density.}\label{fig:mach-1p05-cst-xs-density}
    \end{subfigure}
    ~
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-mach-number-nel-250-plot.eps}
    ...
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-radiation-nel-250-plot.eps}
    \caption{Radiation energy density.}\label{fig:mach-1p05-cst-xs-radiation}
    \end{subfigure}        
    ~
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-visc-nel-250-plot.eps}
    \caption{Viscosity coefficients.}\label{fig:mach-1p05-cst-xs-visc}
    \end{subfigure}        
    \end{center}
\caption{Numerical ($251$ nodes) and semi-analytical ($\sim 3500$ nodes) steady-state profiles ((a), (b), (c) and (d)) along with the viscosity coefficient profiles (e) for Mach $1.05$ test case.}\label{fig:mach-1p05-cst-xs}    
\end{figure}
%\FloatBarrier
%
%------------------------------------------------------------------
...
%------------------------------------------------------------------
%
For this test, the inlet Mach number is set to $3$ and the total and absorption cross sections are still assumed constant. The initial conditions and opacities are computed from the four non-dimensional numbers $\mathbb{P}_\infty=10^{-4}$, $M=3$, $\mathbb{\Sigma}=10^6$ and $\mathbb{\Lambda}=1$ which still yields an opacity of $577.35 \ cm^{-1}$. The code is run with a CFL condition of 1 until the steady-state solution is reached. The purpose of this test is twofolds: (i) it will show that the EVM achieves first-order accuracy for numerical solutions with shocks, and, (ii) will illustrate the point developed in \sct{sec:ent-cond-full-GRH}, i.e. the source terms positively contribute to the stabilization of the numerical solution.

A convergence study is performed for the variables $(\rho, T, \epsilon, Mach)$ by following the method detailed in \sct{sec:mthd-conv-test} on successive meshes ranging from 500 to 1700 cells. The $L_1$ error norms computed with mass ($\Delta \rho$) and energy ($\Delta (\rho E)_{tot}$) conversations are given in \fig{fig:mach-3-cst-xs-conv} when using a semi-analytical solution of $5 \times 10^4$ nodes. A reference line of slope 1 shows that first-order convergence of the $L_1$ error norm computed with mass and energy conversations is achieved when refining the mesh for all variables as expected for a numerical solution with a shock. 
%A closer look at the convergence rate shows that the variables other than the material density require a fine mesh in order to display the expected convergence of rate of 1. The reasons of such behavior remain unclear but could be caused by the non-conservative terms present in the material and radiation energy equations. \tcb{I also believe that one of the reason is that we actually solve for the density whereas we do not solve for the other variables that are just post-processed}
%
\begin{figure}[ht]
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mass-energy-diff-mach-3-density-convergence.eps}
    \caption{Material density.}\label{fig:mach-3-cst-xs-density-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mass-energy-diff-mach-3-mat-temp-convergence.eps}
    \caption{Material temperature.}\label{fig:mach-3-cst-xs-temp-conv}
    \end{subfigure}
    
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mass-energy-diff-mach-3-mach-number-convergence.eps}
    \caption{Mach number.}\label{fig:mach-3-cst-xs-mach-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mass-energy-diff-mach-3-radiation-convergence.eps}
    \caption{Radiation energy density.}\label{fig:mach-3-cst-xs-radiation-conv}
    \end{subfigure}        
\caption{Convergence plots of the $L_1$ error norm for Mach $3$ test with constant opacity.}\label{fig:mach-3-cst-xs-conv}    
\end{figure}
%

The steady-state numerical results obtained with a mesh of $1000$ cells are plotted against the semi-analytical solutions \cite{ferguson} along with the viscosity coefficients $\kappa$ and $\kappa_{max}$ in \fig{fig:mach-3-cst-xs}. All variables show good agreement with the semi-analytical solutions. A shock is located around $x=0$ in the material density and mach number profiles in \fig{fig:mach-3-cst-xs-dens} and \fig{fig:mach-3-cst-xs-mach}, respectively. It is well resolved and does not display any instabilities. In \fig{fig:mach-3-cst-xs-temp}, the material temperature profile displays a Zeldovich's pike in the vicinity of the shock region: the pike is well resolved and matches the semi-analytical solution. On the other hand, the radiation temperature shown in \fig{fig:mach-3-cst-xs-rad} remains smooth even in the vicinity of the peak because of the diffusion term $ \partial_x \left( \frac{c}{3 \sigma_t} \partial_x \epsilon \right)$ in the radiation equation (\eqt{eq:GRHrad}). The viscosity coefficients $\kappa$ and $\kappa_{max}$ are plotted in \fig{fig:mach-3-cst-xs-visc} on a log scale. It is observed that $\kappa$ is peaked in the vicinity of the shock and small anywhere else. This behavior is conformed to the definition of the viscosity coefficient $\kappa$ that is set proportional to the local entropy residual and the jumps. It is also observed that the viscosity coefficient $\kappa$ does not saturate to the first-order viscosity coefficient $\kappa_{max}$ but still efficiently stabilizes the numerical solution. This is a consequence of the point developed in \sct{sec:ent-cond-full-GRH} that highlights the entropy-producing effects, i.e. stabilizing effects, of the relaxation and the diffusion terms (see \eqt{eq:ineq_ent_eq}). 
%Consequently, the relaxation and diffusion terms participate in stabilizing the numerical solution along with the EVM.
%
\begin{figure}[h]
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-mat-temp-nel-1000-plot.eps}
    \caption{Material temperature.}\label{fig:mach-3-cst-xs-temp}
    \end{subfigure}	
    ~
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-density-nel-1000-plot.eps}
    \caption{Material density.}\label{fig:mach-3-cst-xs-dens}
    \end{subfigure}
    ~
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-radiation-nel-1000-plot.eps}
    ...
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-mach-number-nel-1000-plot.eps}
    \caption{Mach number.}\label{fig:mach-3-cst-xs-mach}
    \end{subfigure}
    ~
    \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-visc-nel-1000-plot.eps}
    \caption{Artificial viscosity coefficients.}\label{fig:mach-3-cst-xs-visc}
    \end{subfigure}        
    \end{center}
\caption{Numerical ($1001$ nodes) and semi-analytical ($\sim 10^5$ nodes) steady-state profiles ((a), (b), (c) and (d)) along with the viscosity coefficient profiles (e) for Mach $3$ test.}\label{fig:mach-3-cst-xs}      
\end{figure}
%
%------------------------------------------------------------------
...
We now consider a Mach 3 test in a pure absorber material with the same non-dimensional numbers as in \sct{sec:mach-3-cst-xs} but with an opacity that depends on the material properties, i.e, the density $\rho$ and the material temperature $T$. An expression of the \emph{dimensional} form of the density- and  temperature-dependent opacities is given in \eqt{eq:opacity}:
...
Once again, a convergence study of the $L_1$ error norm is performed for the variables $(\rho, T, T_r, Mach)$ and plots are shown in FIGURE. A reference line of slope 1 shows that first-order convergence of the $L_1$ error norm is achieved for all variables as expected for a numerical solution with a shock.

The test is run until a steady-state solution is reached. Numerical results for the material density and the material and radiation temperatures are presented and plotted against semi-analytical solutions in \fig{fig:mach-3-dpt-xs-dens}, \fig{fig:mach-3-dpt-xs-mat-temp} and \fig{fig:mach-3-dpt-xs-rad-temp}, respectively. The values of the total absorption opacity is given in \eqt{fig:mach-3-dpt-xs-xs}. We also provide the viscosity coefficient profiles at steady state in \fig{fig:mach-3-dpt-xs-visc}.
%\begin{itemize}
...
\begin{figure}[ht]
    ...
    \end{subfigure}        
\caption{Convergence plots of the $L_1$ error norm for Mach $3$ test.}\label{fig:mach-3-dpt-xs-conv}    
\end{figure}
%
...
The EVM was tested with various test cases in a pure absorber material and the numerical solutions were compared against a semi-analytical solutions. It was demonstrated that second- and first-order accuracy were achieved for smooth (Mach 1.05 test case) and with shock (Mach 3 test case) solutions, respectively. Physical features such as embedded hydrodynamic shocks and the Zeldovich spike are resolved accurately without spurious oscillations. The entropy viscosity coefficient is only peaked in the vicinity of the shock and in region of strong gradients, and remains small elsewhere, as expected. Overall, the entropy viscosity method behaves very satisfactorily when compared against semi-analytical solutions.
