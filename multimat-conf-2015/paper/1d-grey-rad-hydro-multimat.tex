...
\title{Viscous regularization of the full {\color{red} set of} nonequilibrium{\color{red} -diffusion} grey radiation-hydrodynamic equations}
...
%-------------------------
\begin{abstract}
{\color{red} JMF comment: Over half of the summary discusses paper [1], and I think gives short-shrift to what is done in this paper, especially since, I think, the summary is meant to summarize the current paper.}

In \cite{our_jcp_radhy_paper}, a viscous regularization technique{\color{red} ,} based on the local entropy residual{\color{red} ,} was proposed to stabilize the nonequilibrium{\color{red} -diffusion} Grey Radiation-Hydrodynamic equations using an artificial viscosity technique. 
The viscosity coefficient is modulated by the local entropy production{\color{red} ,} and peaks at shock locations. 
...
%
Here, we extend the method and derive an entropy minimum principle for the full {\color{red} set of} nonequilibrium{\color{red} -diffusion} Grey Radiation Hydrodynamic equations.
This further strengthens the \st{potential and} applicability of the entropy viscosity method as a stabilization technique for radiation-hydrodynamic shock simulations.
