...
\item
The system of equations without the {\color{red} radiation(?)} source terms cannot be written in a conservative form, and thus the notions of weak and entropy solutions cannot be defined in the sense of distributions \cite{dlm, lefloch_1988, lefloch_1989, lefloch_liu_1993, bianchini_bressan_2005}.
...
This remark is of importance for the case where the GRH devolve to the \tcr{why? conservative} EDA equations.
...
\tcr{We believe our present work is the first time that their semi-analytic solutions have been used for a convergence analysis in situations with temperature-dependent opacities where a Zeldovich spike occurs and we describe the process we used to carry out the convergence analysis.}
...
%------------------------------------------------------------------
%
In this section, we review some mathematical properties of the GRH equations and their viscous regularization.
...
%------------------------------------------------------------------
\subsection{The 1-D regularized nonequilibrium-diffusion GRH equations}
...
The left-hand-side of \eqt{eq:regularized_hyperbolic_GRH} consists of the inviscid 1-D \tcr{nonequilibrium-diffusion} GRH equations, which are obtained by coupling the Euler equations and the radiation momentum and energy equations.
...
%(i.e., the absorption opacity $\sigma_a$ and the diffusion coefficient $D$ are set to zero in \eqts{eq:regularized_hyperbolic_GRH}), 
...
This test yields a smooth solution and thus serves at demonstrating second-order accuracy of the EVM by performing a convergence study in the $L_1$ and $L_2$ error norms. We also use this test to illustrate the shifting procedure. % of the semi-analytical solution detailed in \sct{sec:mthd-conv-test}.
...
%    \end{subfigure}    
    ...
    \end{subfigure}
    \caption{Material density before (a) and after (b) shifting.\tcr{what are the $\Delta \rho$ values in the plots? They do not correspond to any value in Table 1.}}\label{fig:mach-1p05-shift-vs-non-shift}
\end{figure}
%
...
The convergence rates for the material density, the material temperature, the radiation temperature and the mach number are given in \fig{fig:mach-1p05-cst-xs-conv} for the $L_1$ and $L_2$ error norms. % between the numerical and semi-analytical solutions for the variables $(\rho, T, \epsilon, Mach)$. 
...
% under mesh refinement with both mass and energy conservations. 
...
% The code is run with a CFL condition of 1 until the steady-state solution is reached. 
