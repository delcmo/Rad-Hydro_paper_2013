...
\tcr{add a few words about $\kappa_{max}$ and $\kappa_e$ along with equation numbers from previous paper}
...
\hat{A} = 
\begin{bmatrix}
\sddhat + \sdehat\frac{2 \hat{\epsilon}}{\hat{\rho}^2} + \seehat \left(\frac{\hat{\epsilon}}{\hat{\rho}^2}\right)^2 &
\sdehat  + \seehat \left(\frac{\hat{\epsilon}}{\hat{\rho}^2}\right) &
-\frac{1}{\rho} \left( \sdehat  + \seehat \left(\frac{\hat{\epsilon}}{\hat{\rho}^2}\right) \right) \\
\sdehat  + \seehat \left(\frac{\hat{\epsilon}}{\hat{\rho}^2}\right) &
\seehat &
-\frac{\seehat}{\hat{\rho}} \\
-\frac{1}{\hat{\rho}} \left( \sdehat  + \seehat \left(\frac{\hat{\epsilon}}{\hat{\rho}^2}\right) \right) &
-\frac{\seehat}{\hat{\rho}} &
\frac{\seehat}{\hat{\rho}^2} + \srrhat
\end{bmatrix} \, ,
\end{equation}
%
where $\hat{s}$ is the functional form of the entropy function of $(\hat{\rho}, \hat{e}, \hat{\epsilon})$. At this stage of the proof, we need to show that the matrice $\hat{A}$ (\eqt{eq:gen-matrix}) devolves to the matrix $A^*$ in the EDL (\eqt{eq:mat-quad-form-edl}). It will be achieved by (i) reducing the size of the matrix $\hat{A}$ from $3 \times 3$ to $2 \times 2$ and then (ii) demonstrate that each entry of the reduced matrix $\hat{A}$ and the matrix $A^*$ are equal.

The matrix $\hat{A}$ is derived from the GRH system of equations that contains four PDEs. In the EDL, the radiation energy density equation is summed to the material energy equation and then substitute for the algebraic equation $\epsilon=aT^4$. As a result, the last row and column of the matrix $\hat{A}$ can be removed to yield the reduced matrix $\hat{A}_{EDL}$ in the EDL:
%
\begin{equation}\label{eq:gen-matrix}
\hat{A}_{EDL} = 
\begin{bmatrix}
\sddhat + \sdehat\frac{2 \epsilon}{\rho^2} + \seehat \left(\frac{\epsilon}{\rho^2}\right)^2 &
...
variations of the computed offset value $x_{offset}$ as a function of the grid size are also presented in \fig{fig:mach-1p05-x-offset}. 
...
\begin{figure}[h]
    ...
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-mat-temp-convergence.eps}
    \caption{Material temperature.}\label{fig:mach-1p05-cst-xs-temp-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-density-convergence.eps}
    \caption{Material density.}\label{fig:mach-1p05-cst-xs-density-conv}
    \end{subfigure}
    
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-mach-number-convergence.eps}
    \caption{Mach number.}\label{fig:mach-1p05-cst-xs-mach-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-1p05/mach-1p05-radiation-convergence.eps}
...
\begin{figure}[h]
    ...
    \end{subfigure}        
    \end{center}
\caption{Numerical ($251$ nodes) and semi-analytical ($\sim 3500$ nodes) steady-state profiles ((a), (b), (c) and (d)) along with the viscosity coefficient profiles (e) for Mach $1.05$ test.}\label{fig:mach-1p05-cst-xs}    
\end{figure}
%\FloatBarrier
%
%------------------------------------------------------------------
...
%------------------------------------------------------------------
%
For this test, the inlet Mach number is set to $3$ and the total and absorption cross sections are still assumed constant. The initial conditions are computed from the four non-dimensional numbers $\mathbb{P}_\infty=10^{-4}$, $M=3$, $\mathbb{\Sigma}=10^6$ and $\mathbb{\Lambda}=1$ which still yields an opacity of $577.35 \ cm^{-1}$. The purpose of this test is twofolds: (i) it will illustrate the point developed in \sct{sec:ent-cond-full-GRH} and, (ii) will show that the EVM achieves first-order accuracy for numerical solutions with shocks. The code is run with a CFL condition of 1 until the steady-state solution is reached. 

A convergence study of the $L_1$ error norm is performed for the variables $(\rho, T, T_r, Mach)$ and plots are shown in \fig{fig:mach-3-cst-xs-conv}. A reference line of slope 1 shows that first-order convergence of the $L_1$ error norm is achieved for all variables as expected for a numerical solution with a shock.
%
\begin{figure}[ht]
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-mat-temp-convergence.eps}
    \caption{Material temperature.}\label{fig:mach-3-cst-xs-temp-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-density-convergence.eps}
    \caption{Material density.}\label{fig:mach-3-cst-xs-density-conv}
    \end{subfigure}
    
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-mach-number-convergence.eps}
    \caption{Mach number.}\label{fig:mach-3-cst-xs-mach-conv}
    \end{subfigure}
    ~
    \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/cst-xs/mach-3/mach-3-radiation-convergence.eps}
    \caption{Radiation energy density.}\label{fig:mach-3-cst-xs-radiation-conv}
    \end{subfigure}        
\caption{Convergence plots of the $L_1$ error norm for Mach $3$ test.}\label{fig:mach-3-cst-xs-conv}    
\end{figure}
%

The steady-state numerical results obtained with a mesh of $1000$ cells are plotted against the semi-analytical solutions \cite{ferguson} along with the viscosity coefficients $\kappa$ and $\kappa_{max}$ in \fig{fig:mach-3-cst-xs}. All variables show good agreement with the semi-analytical solutions. A shock is located around $x=0$ in the material density and mach number profiles in \fig{fig:mach-3-cst-xs-dens} and \fig{fig:mach-3-cst-xs-mach}, respectively. It is well resolved and does not display any instabilities. In \fig{fig:mach-3-cst-xs-temp}, the material temperature profile displays a Zeldovich's pike in the vicinity of the shock region: the pike is well resolved and matches the semi-analytical solution. On the other hand, the radiation temperature shown in \fig{fig:mach-3-cst-xs-rad} remains smooth even in the vicinity of the peak because of the diffusion term $ \partial_x \left( \frac{c}{3 \sigma_t} \partial_x \epsilon \right)$ in the radiation equation (\eqt{eq:GRHrad}). The viscosity coefficients $\kappa$ and $\kappa_{max}$ are plotted in \fig{fig:mach-3-cst-xs-visc}. It is observed that $\kappa$ is peaked in the vicinity of the shock and small anywhere else. This behavior is conformed to the definition of the viscosity coefficient $\kappa$ that is set proportional to the local entropy residual and the jumps. It is also observed that the viscosity coefficient $\kappa$ does not saturate to the first-order, over-dissipative viscosity coefficient $\kappa_{max}$ but still efficiently stabilizes the numerical solution. This is a consequence of the point developed in \sct{sec:ent-cond-full-GRH} that highlights the entropy-producing effects of the relaxation and the diffusion terms (see \eqt{eq:ineq_ent_eq}). Consequently, the relaxation and diffusion terms participate in stabilizing the numerical solution along with the EVM.
%
\begin{figure}[h]
    \begin{subfigure}{0.32\textwidth}
    \centering
