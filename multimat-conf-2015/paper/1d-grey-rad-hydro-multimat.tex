...
compared with semi-analytical reference solutions by performing a convergence study. \tcb{We propose a procedure to perform the convergence study with a semi-analytical solution, and apply it to show that the expected convergence rates are achieved for all tests when refining the mesh.}
\end{abstract}
%-------------------------
...
Their results show good agreement with the semi-analytical solutions of Lowrie and Edwards \cite{LowrieEdwards} \tcb{within a eyeball norm}.
%
...
conservation laws \cite{Lax}. \tcb{I deleted the sentence on the Borel measure}
...
\tcb{Numerical results and convergence studies are presented in \sct{sec:num-rslt}. We first detail the procedure used for the convergence studies as it has not been published to the best of your knowledge. Then, numerical results for Mach 1.05 and Mach 3 tests are presented.
%The new theoretical result is illustrated in \sct{sec:num-rslt} with numerical results for Mach 1.05 and Mach 3 tests. 
For each test, the numerical solution is compared against semi-analytical solutions by performing a convergence study with the presented procedure. 
...
We recall that, in the remaining of this paper, the opacity is denoted by $\sigma$ and has of units $cm^{-1}$. }
...
Reflective (or zero gradient) boundary conditions are used at both inlet and outlet for the radiation diffusion equation (\eqt{eq:GRHrad}).
...
(through solving Equations (12) and (13) of \cite{LowrieRauenzahn}) to $P_0 \approx 8.5 \cdot 10^{-5}$. The opacities are assumed constant and set to $\sigma_t = \sigma_a = 577.35 \ cm^{-1}$ \tcb{unless otherwise specified.}
...
The integral over the computational domain delimited by $[x_1; x_2]$ is split over $nb_{cells}$ cell integrals. Each cell integral is numerically computed with a quadrature rule (index $q$ in \eqt{eq:l1-error-norm}); a Gauss quadrature rule of order $10$ is used here. This means that the semi-analytical and numerical solutions be evaluated at quadrature points. The numerical solution is computed at quadrature points using the finite element expansion (we used Lagrange test functions in this paper). The values of the semi-analytical solution at quadrature points are computed with a linear interpolation.
%(Fortran version of the function '\emph{interp1d}') \tcr{who knows what interp1d is. why don't you say why is used actually?}\tcb{I think I should just remove it}. 
%>>>>>>> 80a5074596b6d65166c9c071f44d3b9efb860727
A typical semi-analytical solution with shock is solved on a non-uniform mesh discretized with about $10^4$ nodes (compared to about $1000$ nodes for the numerical solution); this should minimize the error due to the interpolation in the semi-analytical solution. Finally, we have chosen to shift the semi-analytical solution since it is computed on a finer mesh than the numerical solution. The exact same procedure can be performed for conservation of total energy by replacing $\rho_h$ and $\rho^\star$ with $(\rho E)_{tot,h}$ and $(\rho E)^\star_{tot}$, respectively, in \eqt{eq:mass-diff}. \tcb{In the remaining of this paper, we refer to mass and energy conservation procedures as the methods used to compute the shifts under conservation of mass and total energy, respectively.}

A spatial convergence study is performed for each test case presented in this paper, which requires to run multiple times the same numerical simulation on successively finer meshes. \tcb{For each new run, once the steady-state solution is obtained, a value of the shift, $x_{\text{\text{offset}}}$, is computed using mass and energy conversations, and used to compute the convergence rates. We present the shift values for conservation of total mass and conservation of
total energy for the Mach 1.05 test case only for illustration purpose.}
...
A convergence study is performed for the variables $(\rho, T, \epsilon, Mach)$ by following the method given previous in \sct{sec:mthd-conv-test} on successively refined meshes \tcb{with spatial step sizes ranging from $\sim 10^{-5}$ to $\sim 10^{-3}$ $cm$}. The $L_1$ error norms computed with mass ($\Delta \rho$) and energy ($\Delta (\rho E)_{tot}$) conversation procedures are given in \fig{fig:mach-3-cst-xs-conv}; the semi-analytical solution used $5 \times 10^4$ nodes. A reference line of slope 1 shows that first-order convergence of the $L_1$ error norm computed with mass and energy conversation procedures is achieved when refining the mesh for all variables as expected for a numerical solution developing a shock \cite{lowrie-2009} (note that only convergence plots for the material temperature are provided in \cite{lowrie-2009}). \tcb{It is also observed in \fig{fig:mach-3-cst-xs-radiation-conv} that the radiation energy density is second-order accurate for $\Delta x \geq 10^{-4}$ even though the material variables achieve first-order accuracy. This unexpected behavior is under further investigation. The beginning of a explanation would be to remember that the radiation energy density remains smooth through the shock and thus could achieve second-order accuracy. However, it is also coupled through the PDEs to the material variables that achieve at best firsts-order accuracy in the presence of a shock. I AM NOT SATISFIED BY THESE LAST TWO SENTENCES}
%but an hypothesis can be laid down: the radiation energy density remains smooth because of the diffusion term and thus should achieve second order accuracy, but is also function of the material variables through the PDEs that experience discontinuities 
...
We now consider a Mach 3 test in a pure absorber material with the same initial conditions as in \sct{sec:mach-3-cst-xs} but with an opacity that depends on the \tcb{a-dimensional} material properties, i.e, density $\hat{\rho}$ and temperature $\hat{T}$. An expression of the \emph{dimensional} form of the density- and  temperature-dependent opacities is given in \eqt{eq:opacity}:
...
Once again, even though the entropy viscosity coefficient $\kappa$ does not saturate to the first-order viscosity coefficient $\kappa_{max}$, the numerical solution is still well stabilized thanks to the stabilizing effect of the source terms, as explained in \sct{sec:VR_new}.
...
The EVM was tested with various test cases and the numerical solutions were compared against semi-analytical solutions obtained by following the method described in \cite{LowrieEdwards}. After proposing a mass and a energy conservation procedures used for the convergence study, we show that second-order accuracy is obtained for smooth solutions (Mach 1.05 test case) and that solutions with shock (Mach 3 test cases) yield first-order accuracy in all variables for fine meshes. \tcb{For the Mach 3 test cases, it was also observed that the radiation energy density achieves second order accuracy on coarse meshes ($\Delta x \geq 10^{-4}$ $cm$), even though a shock develops in the material variables. Such behavior is unexplained at this date but is under further investigation.}
