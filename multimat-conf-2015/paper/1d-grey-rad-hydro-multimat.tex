...
proposed to solve the non-equilibrium Grey Radiation-Hydrodynamics 
...
%------------------------------------------------------------------
%
The definition of the artificial entropy viscosity coefficient $\kappa$ is identical to what was previously used in \cite{our_jcp_radhy_paper}. We thus refer the reader to Section 2.4 of \cite{our_jcp_radhy_paper} for further details regarding the derivation of the entropy viscosity coefficient $\kappa$ and its definition.
...
In the EDL, the GRH system of equations devolve to a system of three PDEs: a mass equation, a momentum equation and a radiation-material energy equation. The radiation-material energy equation is obtained from the summation of the radiation energy density equation, \eqt{eq:GRHrad} and the material energy equation, \eqt{eq:GRHenerg}.  \eqt{eq:GRHenerg} is then substituted for the closure relation $\epsilon = aT^4$. Consequently, the entropy is function of only two variables and the resulting quadratic form
...
Using the same approach, it can be proved that:
%
...
In the EDL, the radiation energy density equation is summed to the material energy equation and then substitute for the algebraic equation $\epsilon=aT^4$. As a result, the last row and column of the matrix $\hat{A}$ can be removed to yield the reduced matrix $\hat{A}_{EDL}$ in the EDL:
%
\begin{equation}\label{eq:gen-matrix}
\hat{A}_{EDL} = 
\begin{bmatrix}
...
\end{equation}
%
In the EDL,i.e. $\hat{x} \to x^*$, using results from \eqt{eq:gen-matrix} and \eqt{eq:chg-var}, it is concluded that the GRH entropy residual given in \eqt{eq:app_entr_eq_non_equil} devolves to the ED entropy residual given in \eqt{eq:EDL-entropy}. This mathematical proof shows that the entropy condition still holds in the EDL which ensures convergence of the numerical solution to the weak physical solution in the same limit.
