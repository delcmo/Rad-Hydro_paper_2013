...
The definition and value they choose for their opacity is similarly arbitrary, and can be combined with their definition for $P_0$ to arrive at a set of values for $T_{\infty} = 100 eV$, such that $P_0 \approx 8.8\cdot 10^{-5}$, and $\sigma = 577.35 \ cm^{-1}$ which are consistent for an ideal-gas equation-of-state. \tcb{Jim: add rho infinity value} We recall that in the case of a purely absorbing material the total and absorption opacities are identically equal; in this paper, all of the simulations presented correspond to this case. \tcb{add reference with section to compute the initial conditions}
