...
Their results \tcr{visually} showed good agreement with the semi-analytical solutions of Lowrie and Edwards \cite{LowrieEdwards}.
\tcr{they didn't do a convergence, right?} \tcb{This is correct.}
%
...
to carry out the convergence studies; \tcr{our present work is the first illustration of such a procedure} \tcb{Done}. Then, numerical results for 
...
%\FloatBarrier
%
\tcr{I do not understand the delta rho in figure 3.b, the num. sol. is obtained with 250 cells). delta rho is about $3e-6$. 
However, in figure 1.b,t he shifted delta rho is $1e-19$. Why isn't the graph 3.b for the SHIFTED numerical solution????   
} \tcb{good catch, it is the delta rho when using the L1 norm to compute the shift. I updated the plots, and removed the delta rho even though they visually look the same}
\pagebreak
...
This behavior may warrant further analysis. (not sure if that very last sentence is useful)
} \tcb{ l like it}
%
\begin{figure}[ht]
    \begin{subfigure}{0.5\textwidth}
...
\begin{figure}[h]
    ...
    \end{subfigure}        
    \end{center}
\caption{Numerical ($1000$ cells) and semi-analytical ($\sim 5 \cdot 10^4$ nodes) steady-state profiles of the material temperature (a), the density (b), the radiation temperature (c) and the mach number (d) along with the viscosity coefficient profiles (e) for Mach $3$ test with constant opacity.}\label{fig:mach-3-cst-xs}      
\end{figure}
%
\pagebreak
...
We now consider a Mach 3 test in a pure absorber material with the same initial conditions as in \sct{sec:mach-3-cst-xs} but with an opacity that depends on the \tcb{a-dimensional} material properties, i.e., density $\hat{\rho}$ and temperature $\hat{T}$. An expression of the \emph{dimensional} form of the density- and  temperature-dependent opacity is given in \eqt{eq:opacity}:
...
\begin{figure}[ht]
    ...
    \end{subfigure}        
\caption{$L_1$ error norm for Mach $3$ test with temperature-dependent opacity.}\label{fig:mach-3-dpt-xs-conv} 
\end{figure}
%
\begin{figure}[h]
    \begin{subfigure}{0.32\textwidth}
    ...
    \end{subfigure}      
    \end{center}  
\caption{Numerical ($2700$ cells) and semi-analytical ($\sim 5 \cdot 10^4$ nodes) steady-state profiles of the material temperature (a), the density (b), the radiation temperature (c) and the mach number (d) along with the viscosity coefficient profiles (e) for Mach $3$ test with temperature-dependent opacity.}\label{fig:mach-3-temp-dep-xs}    
\end{figure}
%
