...
{\affilnum{2}Los Alamos National Laboratory, Los Alamos, NM 77843, USA}
}
...
for radiation-hydrodynamic shock simulations, in \tcb{TO DELETE both} the streaming \tcb{TO DELETE: and equilibrium-diffusion} limits. 
...
\end{subequations}
%
where $\rho$, $\rho u$, $\rho E$, $P$, and $T$ are the material density, momentum, total energy, pressure, and temperature, respectively, and $\epsilon$ denotes the radiation energy density. The variable $\kappa$ is an artificial viscosity coefficient whose definition will be \tcb{recalled}  later in \sct{sec:visc-coeff-def}.
...
local entropy production \tcb{ and jumps}. The entropy residual $R_e$ of \eqt{eq:app_entr_eq_non_equil} can be recast
...
demonstrate below \tcb{TO DELETE:, in \sct{sec:VR_new}}, the contribution from the relaxation and diffusion terms to the entropy residual equation will only 
...
(through solving Equations (12) and (1) of \cite{LowrieRauenzahn}) to $P_0 \approx 8.5 \cdot 10^{-5}$. \tcr{last question: how do we get $\sigma = 577.35 \ cm^{-1}$ ?} \tcb{you still need to give the opacity value}
\\
...
total energy \tcb{for the Mach-1.05 test case presented in \sct{sec:mach-1p05-cst-xs}}.
