...
\end{subequations}
%
where $P^*=P+\frac{1}{3}aT^4$ and $E^*=e^*+0.5u^2$ with $e^*=e+a\frac{T^4}{\rho}$.
It was not, however, demonstrated that the entropy equation for the regularized GRH recalled in \eqt{eq:visc_reg_assumptions}, devolves to the entropy equation that would be obtained when directly starting from \eqt{eq:equip-diff-equ}.
\tcb{Add entropy equation}
%
%\begin{equation}\label{eq:GRH-entropy}
%\rho R_e(x,t) = \rho \frac{D s}{Dt}  =\rho  \left( \partial_t s + u \partial_x s \right) \geq 0 \text{ where } R_e(x,t) = \partial_t s + u \partial_x s \, .
%\end{equation}
%%
...
yield positive terms when using the functional form of the entropy function given in \eqt{eq:ent_equ}. It is also demonstrated in \sct{sec:cons-equi-diff-limit} that in the EDL and by performing a change of variable to come, the entropy equation obtained for the full GRH and recalled in \eqt{eq:app_entr_eq_non_equil}, devolves to the entropy equation obtained from \eqt{eq:equip-diff-equ}. Such a behavior was expected bur remained to be proved. These two mathematical proofs will enhance the understanding of the EVM and will also allow us to further explain in \sct{sec:num-rslt} some of the features observed in the original numerical results presented in \cite{our_jcp_radhy_paper}. \\
...
%------------------------------------------------------------------
%
In \cite{our_jcp_radhy_paper}, the viscous regularization for the GRH equations is shown to yield well scaled viscous terms in the equilibrium-diffusion limit \tcb{add section}. It is proposed here, to show that the \tcb{add the entropy equations of the two systems for clarity so that the reader can understand where we are going}
entropy equation derived for the regularized GRH, \eqts{eq:regularized_hyperbolic_GRH}, devolves to the entropy equation that would be obtained from the equilibrium-diffusion equations. Such behavior is expected but proof of it was not provided in \cite{our_jcp_radhy_paper}. We refer the reader to APPENDIX for details of the proof and only present in this section the main steps as the proof is lengthy. 

In the Equilibrium Diffusion Limit (EDL), the system of equations, \eqt{eq:equip-diff-equ}, is not longer expressed as a function of $\left[ \rho, \rho u \rho E, \epsilon \right]$ but as a function of $\left[ \rho, \rho u, \rho E^* \right]$ along with the closure relation $\epsilon = aT^4$. In the EDL, the GRH system of equations devolve to a system of three PDEs: the radiation energy density equation, \eqt{eq:GRHrad} is summed to the material energy equation, \eqt{eq:GRHenerg}, and is substituted for the closure relation $\epsilon = aT^4$. Consequently, the quadratic from of the entropy equation in the EDL is only function of two variables $\left[ \rho, e^* \right]$ and yields a $2 \times 2$ matrix. In order to prove that the entropy equation derived from the GRH equations devolves to the correct entropy equation in the EDL, a two-step process is proposed. First, the following change of variable is performed in the entropy equation:
%
...
\end{equation}
%
where $\hat{e} = e + \frac{\epsilon}{\rho}$. Then, each term of the resulting entropy equation are derived in the EDL. Finally, the entropy equation is compared to the entropy equation obtained from \eqt{eq:equip-diff-equ}. Note that the change of variable proposed in \eqt{eq:chge_var} is consistent with the EDL when using the closure relation, $\epsilon = a T^4$, which yields $\hat{e} = e^* = e + \frac{a T^4}{\rho}$ as previously defined in \sct{sec:ent-min}. 
