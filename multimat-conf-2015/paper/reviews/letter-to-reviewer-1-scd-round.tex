...
Regarding combustion and flame modeling, we believe that frequency-dependent equations are typically solved to obtain photon spectral information; here we solve the grey (frequency-integrated) equations. Moreover, the GRH contain 
``$u/c$'' terms (e.g., $u/(3c) \partial_x \epsilon$, where I divided by $c$ to make the terminology ``$u/c$'' obvious). These terms tyically come into play when temperatures exceed the million-degree mark and the material kinetic energy is significantly affected by radiation momentum deposition. 
