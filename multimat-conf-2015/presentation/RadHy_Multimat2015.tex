...
\begin{itemize}
\item Initial attempt: apply the EVM to  we first applied the hyperbolic part of the GRHD [an idea similar to \tcb{Balsara JQSRT 1999}, \tcb{Lowrie\&Morel JQSRT 2001}]
\end{itemize}
\item
...
%
\begin{equation}
\partial_t \left( \rho E\right) + \partial_x \left[ u \left( \rho E + P \right) \right] +\frac{u}{3} \partial_x \epsilon = \tcr{\partial_x \left( h \right) }
\end{equation}
%
\begin{equation}
...
\item With \tcr{$s(\rho,e,\epsilon)$}, use chain rule to obtain the entropy relationship
	...
	\end{equation} 
\item Simplify the expression by assuming \tcr{$s(\rho,e,\epsilon) = s_{Euler}(\rho,e) + s_{rad}(\rho,\epsilon)$}
...
\end{equation}
We recover the EDL results. \\
Viscous regularization scales adequately.
\end{block}
...
\item semi-analytical solutions provided by Jim Fergusson
\item Two sets of results presented today:
\begin{enumerate}
\item with constant opcacities (Mach 1.05, 2, 5, 50)
...
\begin{itemize}
\item Extended the entropy-viscosity method to \tcm{full} Grey Radiation-Hydrodynamic equations
\item Verified entropy minimum principle for regularized equations
\item Viscous regularization scales appropriate in the equilibrium-diffusion limit
...
\item switch solution technique to IMEX
\item DGFEM spatial discretization
\end{itemize}
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Why an upper bound for viscosity?}[noframenumbering]
