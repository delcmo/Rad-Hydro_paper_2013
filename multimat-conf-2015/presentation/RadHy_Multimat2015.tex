...
By proving that the \tcr{regularized} equations satisfy a minimum principle on the specific entropy, \tcr{$s(e,\rho)$}  \tcb{[Guermond/Popov/Pasquetti (JCP 2011)]}

\end{block}

\begin{block}{Entropy minimum  principle}
\be
...
\begin{block}{Entropy relationship: $\rho ( \partial_t s + \vec{u} \cdot \grad s ) = \ldots  \tcr{\ge 0}$}
...
\item Entropy viscosities $\propto$ entropy production
\item All-speed (from low-Mach to supersonic) extension by \tcb{Delchini/Ragusa/Berry, Computers \& Fluids, 2015} 
%\item
...
\begin{enumerate}
\item 
the GRHD equations are not hyperbolic
\item
What is an appropriate functional form for the entropy of the GRHD, $s(\rho,e,\epsilon) = \ldots$? 
\item
What is an appropriate expression for the viscous fluxes so that the \tcr{regularized} GRHD eqs satisfy 
the entropy minimum principle?
\item
Is the viscous regularization well-behaved in the equilibrium-diffusion limit?
\end{enumerate}
\end{block}

\begin{block}{Study of the hyperbolic part of the GRHD}
Several numerical techniques are based on the characteristics of the hyperbolic parts of the GRHD (Balsara JQSRT 1999, Lowrie\&Morel JQSRT 2001)
\begin{subequations}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Entropy-based artificial viscosity technique for the GRHD: outline of the derivation}

We add viscous regularization terms:
\begin{subequations}
\begin{equation}
...
%
\begin{equation}
\partial_t \left( \rho E\right) + \partial_x \left[ u \left( \rho E + P \right) \right] +\frac{u}{3} \partial_x \epsilon = \tcr{\partial_x \left( h + ug \right) }
\end{equation}
%
\begin{equation}
\partial_t \epsilon + \frac{4}{3} \partial_x \left( u \epsilon \right) - \frac{u}{3} \partial_x \epsilon = \partial_x \ell
\end{equation}
\end{subequations}

Use chain rule to get an entropy equation 
\begin{equation}
...
\begin{equation}
s(\rho, e, \epsilon) = s_{\Euler}(\rho,e) + s_{rad}(\rho, \epsilon) 
                     = s_{\Euler}(\rho,e) + \frac{\beta}{\rho} \epsilon^\frac{3}{4}
\end{equation}
with the following viscous fluxes

We introduce the following viscous fluxes:
\begin{equation}
\left\{
 \begin{array}{ccc}
  f &= \kappa \partial_x \rho \\
  g &= \rho \mu \partial_x u + uf \\
  h &= \kappa \partial_x (\rho e) - \tfrac{1}{2} u^2 f\\
...
We variables is expanded in a power series in $\varepsilon$, e.g.,
%
...
leading-order continuity and momentum equations, and the second-order total energy equation, i.e.,
%
\begin{subequations}
\begin{equation}
\partial_t \rho_0 + \partial_x \left( \rho u \right)_0 = \partial_x \left( \kappa \partial_x  \rho \right)_0  \ ,
...
In JCP 2015, we studied the hyperbolic art of the GRHD:

We obtained the following functional form of the entropy:

shown that the speed of sound was  ... 

And the regularized hyperbolic part of the GRHD read as follows:

We shown that these equations lead to the following entropy inequality ($\geq 0$)

Thus, we used this VR for the full GRHD:

The viscous regularization is well behaved in the asymptotic ED limits, show EDL equations  

Show that we recover the EDF entropy functional form

For viscosity coefficients, we employed the same strategy as before: min LLF, entro with entro propto entro residual

constant opacity results: Mach 1.05 to 50


new theoretical results.

VR satisfies entropy min principle  for full GRHD
