function [ diff_rhou ] = compute_shift_with_total_energy_conservation(numeric_filename,options,...
...
a=options.eos(3);
Pinf=options.eos(4);
% rhoE_star_num = Pinf*numeric_value(:,3);
% % % rhoE_star_num = rhoE_star_num/rhoE_star_num(1,1);
% rhoE = rho*Cv*T*[1+0.5*M^2*gamma*(gamma-1)]
% rhoE_star_num = rhoE_star_num + numeric_value(:,2); %  rhoE_star_num + Cv*numeric_value(:,2).*numeric_value(:,5).*...
% %     ( ones(size(numeric_value(:,2))) + 0.5*gamma*(gamma-1)*numeric_value(:,4).*numeric_value(:,4) );
% rhoE_star_num = rhoE_star_num/rhoE_star_num(1,1);
% 
% rhoE_star_exact = Pinf*exact_value_unique(:,3);
% % rhoE_star_exact = rhoE_star_exact/rhoE_star_exact(1,1);
% rhoE_star_exact = rhoE_star_exact + Cv*exact_value_unique(:,2).*exact_value_unique(:,5).*...
%     ( ones(size(exact_value_unique(:,2))) + 0.5*gamma*(gamma-1)*exact_value_unique(:,4).*exact_value_unique(:,4) );
% rhoE_star_exact = rhoE_star_exact/rhoE_star_exact(1,1);
...
node_cell_coord = rhou_num(i_cell:i_cell+1,1);
...
while (rhou_exact(index_left,1)<node_cell_coord(1))
    index_left = index_left+1;
end
index_right = index_left;
% while (rhoE_star_exact(index_right,1)<node_cell_coord(end))
while (rhou_exact(index_right,1)<node_cell_coord(end))    
...
exact_xq(:,1) = interp1(exact_value_unique(index_left:index_right,1), rhou_exact(index_left:index_right,1), xq_coord, options.interpolation_type);
